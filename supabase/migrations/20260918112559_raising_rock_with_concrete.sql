...
-- slides off it: a house-floor on a cliff was a hollow you could not fill.
-- Mortar worked stiff with ashes is concrete, mixed by hand at masonry's
-- bench, and a lot of it laid with a trowel raises a bare rock corner by one.
-- On soil it is refused — concrete goes on bare rock — and under water it
-- will not set. The slope it would make is the mason's to answer for exactly
-- as a spadeful's is the digger's: `max_mason_slope` is `max_dig_slope` with
-- the other skill in it. Concrete that slumps off is concrete gone.
--
-- `raise_rock` joins the ground family beside `drop_dirt`, whose mirror it
-- is: the height goes up and the soil over the corner stays nought, so the
-- rock rises rather than the dirt.

/** The same rule for a mason raising rock as for a digger moving soil. */
create or replace function max_mason_slope(p_world uuid, p_uid uuid) returns integer
  language sql stable as $fn$
  select greatest(40, floor(skill_of(p_world, p_uid, 'masonry') * 3)::int)
$fn$;
...
AS $function$
  select p_action in ('flatten', 'drop_dirt', 'pave_slabs', 'remove_paving',
                      ...
                      'plant', 'prune', 'graft', 'harvest_bush', 'dig_stump', 'raise_rock', 'dig_worms', 'prospect')
$function$
;
