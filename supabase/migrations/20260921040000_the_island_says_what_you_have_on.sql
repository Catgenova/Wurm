-- The island says what you have on
--
-- Reported: *"there's no option to unequip gear, even dropping and picking it
-- up indicates that i'm still wearing it."*
--
-- Two faults, one on each side of the wire, and this migration is the island's
-- half.
--
-- `player.equipped` is a jsonb column of the player row. `rpc_do` has written
-- to it on every `equip` and every `unequip` since the day those two were
-- dispatched here -- `jsonb_set` on the way in, `equipped - slot` on the way
-- out -- and `rpc_settle` has never sent it back. So the browser's copy of
-- what you are wearing was whatever that tab had done to itself since it
-- opened, and on a fresh page that is nothing at all.
--
-- What that looks like to somebody playing: you put on a helm, the island
-- writes it down, you reload, and the helm is in your pack unmarked. The item
-- menu asks `applies` of both gear actions -- `equip` wants the thing *not* to
-- be worn and `unequip` wants it to be -- so with an empty record the menu
-- offers `Wear or wield` on everything and `Take it off` on nothing. There is
-- no way back off, short of taking the armour to pieces.
--
-- One line, in the block written for exactly this class of fault three
-- migrations ago: the columns the row has always kept and never said.
