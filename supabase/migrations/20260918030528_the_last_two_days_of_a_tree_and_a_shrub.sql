...
--
-- Asked from the island, three stages at once, and the byte had room for them
-- from the change before this one.
--
-- A tree lived four days: sapling, young, mature, old, gone. It lives six now.
-- After old comes *very old* — the biggest timber there is, four strokes and
-- five logs, still bearing, still a sprout in it, and the last stage a hatchet
-- can take back — and after that *shrivelled*: dead wood standing up. Nothing
-- hangs on a shrivelled tree, nothing sprouts from it, there is no pruning it,
-- and two strokes bring down what timber is left in it. When its day is up it
-- goes, and leaves what an old tree always left.
--
-- And *clipped*: a sapling pruned back, a shrub for good. It never grows and
-- never dies, which is written down as a stage whose next stage is itself —
-- `tree_day` learned to leave such a tree alone in the change before this one,
-- before any such tree existed.
--
-- ## Where the rules are
--
-- All in `tree_age_def`, which the generated definitions carry across from
-- `TREE_AGES`: what each stage becomes, what it prunes to, how many strokes,
-- how many logs, whether it bears, and now whether there is life in it. The
-- three doors that had to learn a dead tree — pruning, a sprout, fruit — ask
-- that last column, and refuse in the browser's words. A very old tree carries
-- fruit the way an old one does, which is the one place a stage is named
-- rather than read: the count is not a column, and two ages is not yet a
-- table.
