-- What you are carrying, which has never gone out at all.
--
-- Asked plainly: "are health, wounds, stamina, hunger, thirst now live on the
-- client?" Four of the five were a question about lateness. The fifth was not.
--
--     doors that mention wounds:    (none)
--     wounds in the net layer:      (none)
--
-- The island has kept wounds since they were ported. `wounds_settle` drains
-- your health from them by `wound_drain` a second, rolls `fester_chance` to
-- turn them bad, closes them over at `wound_close`, and tells you when either
-- happens — "Something you have been carrying has gone bad. Lye, and then a
-- dressing." All of it real, all of it in `player.wounds`, and none of it has
-- ever been said to a browser.
--
-- So on a live island the wound window was empty however cut about you were,
-- `bleeding()` was false, and a bandage or a healing cover had nothing to be
-- put on — while your health drained for no reason the screen could give. The
-- same shape as the seven item fields: the island knew, and nobody asked.
--
-- Sent every beat rather than when they change, unlike the skill book beside
-- it. That one is an aggregate over a table and worth skipping; this is a
-- column of a row already read, so there is nothing to save — and `[]` has to
-- be able to mean "they have all closed over", which a key left out cannot.
