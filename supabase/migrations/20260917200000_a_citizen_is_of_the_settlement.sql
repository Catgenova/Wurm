-- A citizen is of the settlement, and everything but the totem knows it
--
-- Asked from the island: *"for now all citizens of a deed should be able to
-- place items and modify everything except the totem."*
--
-- Half of that was already true and the other half had never been, and the
-- seam between them is worth writing down because it is the shape of a dozen
-- bugs on this island.
--
-- `deeds_of` has always counted citizens:
--
...
--
-- and everything that goes through it — `my_deed`, `deed_here`, `on_my_deed`,
-- and so `plan_reason`, the placing rules, the paving, the digging — has
-- treated a citizen as being at home since the day citizens landed. So did
-- `rpc_ground`, which is where the browser gets its idea of *its* deed: a
-- citizen's browser has the settlement's borders and offers everything inside
-- them.
--
-- Seventeen rules did not use it. They asked the question by hand —
--
--     exists (select 1 from deed where world_id = p_world and founded_by = p_uid)
--
-- — which is not "have you a settlement" but "did you found one". A citizen
-- got a browser that offered the work and an island that answered no.
--
--     assign_deed      You have no settlement to assign it to.
--     breeding         Found a settlement first: the young one goes to the token.
--     bounty           You have no settlement, and so no fields.
--     recall           You have nowhere to be recalled to.
--     a companion      and no settlement to keep another.
--     worker_cap       nought, so a citizen could set nobody to work
--
-- And the same by-hand question decided where a *beast* calls home, keyed on
-- its keeper — so a citizen's stored wildermon, anything born to one, and
-- anything called off a work post had no token to stand by and stayed where
-- it was dropped.
--
-- All eighteen ask `my_deed()` now, which is `deeds_of` and so is founder
-- first, citizen otherwise.
--
-- ## What is still the founder's
--
-- The totem, and nothing else:
--
--     upgrade_deed, disband_deed, rename_deed   settlement_refusal
--     upgrade_reason, upgrade_wants             what the next level wants
--     place_deed_crate                          the crate beside the token
--     found_settlement                          a stake of your own
--     rpc_invite, rpc_leave_deed, rpc_social    who is a citizen at all
--
-- Those keep `founded_by` deliberately. A citizen may build the town, work it,
-- pave it and keep its beasts; the token is the founder's to raise, name,
-- upgrade and pull down.
...
begin
  ...
  if v_dam.mode = 'stored' then
    select md.x + 0.5 as x, md.y + 1.5 as y into v_home from my_deed(p_world, v_dam.keeper) md where md.world_id is not null;
...
begin
  ...
  update creature set hitched_to = p_piece, carrying = null, enemy = null, hunting = null,
      ...
      mode = case when c.mode = 'stored'
                  then case when (my_deed(p_world, c.keeper)).world_id is not null
...
begin
  ...
  select * into dd from my_deed(p.world_id, p.made_by) md where md.world_id is not null;
...
begin
  ...
  on_deed := on_my_deed(p_world, p_uid, tx, ty);
  if on_deed then
    quiet := quiet * 0.7;
...
begin
  ...
  if p_action = 'bait_trap' then
    ...
    if (my_deed(p_world, p_uid)).world_id is null
...
CREATE OR REPLACE FUNCTION public.worker_cap(p_world uuid, p_uid uuid)
 ...
 STABLE
AS $function$
  select coalesce((my_deed(p_world, p_uid)).level, 0)
$function$;
...
-- perform_settlement (1 -> 0, and five left standing: the totem's own)
