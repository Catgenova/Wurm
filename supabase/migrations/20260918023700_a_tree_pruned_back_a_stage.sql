-- A tree pruned back a stage
--
-- Asked from the island: add Prune on trees, to take their age back one stage.
-- Only on old and mature trees.
--
-- A stage back apiece. An old tree that would be gone when its day is up is a
-- mature one instead, and a mature one is young again and stops bearing until
-- it grows back — so pruning is how a wood, or an orchard, is kept from the
-- day that takes it, at the cost of a day's fruit. The forester's hatchet does
-- it, and the check is forestry's.
--
-- ## Where the rule is written
--
-- `tree_age_def.pruned` — what each age prunes to, or null for one too young.
-- Written down beside `next` rather than worked backwards from it, because the
-- stored ages are in the order they were added and not the order a tree lives
-- them (a sapling is 3 and grows into 0), and a rule that has to know that in
-- two places is a rule that will be got wrong in one of them. It comes across
-- from `TREE_AGES` in the generated definitions, so the browser's menu and the
-- island's door read the same column.
--
-- The age bits and nothing else change. The species stays, and so does the
-- notch a hatchet has left in the trunk: pruning is the crown's business, and
-- a half-felled tree pruned back is still half felled.
--
-- `prune` joins the ground family, beside `pick_sprout` and `plant`: the reach
-- and the "you need a hatchet" are the family's, and `act_ported` already
-- answers for the family, so nothing there is touched.
...
AS $function$
  select p_action in ('flatten', 'drop_dirt', 'pave_slabs', 'remove_paving',
                      ...
                      'plant', 'prune', 'dig_worms', 'prospect')
$function$
;
...
begin
  ...
  if p_action = 'flatten' then
    ...
    /*
     * The age bits and nothing else. The species stays, and so does the notch
     * a hatchet has left in the trunk: pruning is the crown's business, and a
     * half-felled tree pruned back is still half felled.
     */
    perform land_set_data(p_world, tx, ty,
      (v_data & 15) | ((v_to.id & 3) << 4) | (tree_cuts(v_data) << 6));
    perform land_announce(p_world, tx, ty);
