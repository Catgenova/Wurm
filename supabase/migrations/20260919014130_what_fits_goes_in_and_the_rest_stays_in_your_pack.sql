...
--
-- It was all or nothing on both sides, everywhere a thing could be put away:
-- a crate, a chest, a bin, a bag. An armful that would not fit whole came back
-- whole with "the crate is full", and the way through it was to count the
-- difference yourself, split the stack by hand, and put in exactly as much as
-- would go. That is arithmetic a game should be doing.
--
-- So a put takes what there is room for and leaves the rest where it was, and
-- says how much stayed behind rather than letting you work it out by looking:
-- "You put 13 x rock shards in the plank crate (pine). The other 35 would not
-- fit." Only a container with no room at all refuses now, and it still refuses
-- by name. A thing that does not stack is one row rather than a number, so it
-- goes in whole or not at all, which is what it always was.

/** How much more a crate will take. */
create or replace function crate_spare(p_world uuid, c crate) returns int
  language sql stable as $fn$
  select greatest(0, crate_capacity(c) - crate_units(p_world, c.id))
$fn$;

/** How much more a chest, a bin or a cupboard will take. */
create or replace function furniture_spare(p placed) returns int
  language sql stable as $fn$
  select greatest(0, furniture_capacity(p) - furniture_units(p))
$fn$;

/** And how much more a bag on your back will take. */
create or replace function bag_spare(b item) returns int
  language sql stable as $fn$
  select greatest(0, bag_room(b.def) - bag_units(b.id))
$fn$;

/**
 * What a put says, including what would not go in. Written once because the
 * browser says the same sentence, and a sentence said twice is a sentence that
 * drifts.
 */
create or replace function stored_line(p_count int, p_what text, p_where text, p_left int)
  returns text language sql immutable as $fn$
  select 'You put ' || case when p_count > 1 then p_count || ' × ' else 'the ' end
    || lower(p_what) || ' in the ' || lower(p_where) || '.'
  ...
  if p_action = 'stow_item' then
    ...
    if p.id is null then
      ...
      loop
        ...
        if v_why is not null then return v_why; end if;
        if furniture_spare(v_other) <= 0 then
