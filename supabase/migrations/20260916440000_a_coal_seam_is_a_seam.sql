-- A coal seam is a seam, and the island could not see one.
--
-- Reported from the island: *"I can't seem to find any Coal ore tiles on the
-- crescent island."* They are there. Coal is in the ore ladder at a share of
-- about one tile of seam in nine, and no region owns it, so `rockKindFor`
-- never turns it into iron — the Crescent has as much of it as anywhere.
--
-- ## The whole of it is one string test
--
--     insert into rock_def values (..., yields.endsWith('_ore'));
--
-- That is `ore`. Not "is this metal", not "is this worth mining" — *does the
-- name of what it gives up end in `_ore`*. A coal seam yields plain `coal`,
-- so it is false, and every rule that goes looking for something worth mining
-- asks that column:
--
--     perform_ground      prospecting sweeps `where (bedrock_at(...)).ore`
--     perform_ground      standing on one: "Plain coal seam beneath you, with
--                         no metal in it"
--     worker_gatherable   a miner works `rock.ore`; a quarrier `not rock.ore`
--     worker_do           `kind = 'mine' and rock.ore then rock.yields`
--
-- So a prospector sweeping forty tiles of coal country is told there is no
-- sign of metal; standing on a seam, the island tells him there is nothing in
-- it; and a wildermon set to mine one is not allowed to, while one set to
-- quarry it brings back rock shards. The one thing that always worked was
-- mining it by hand — `act_perform` reads `r.yields` without asking `ore` —
-- which is why the coal exists at all and nobody could find it.
--
-- ## Two questions, two columns
--
-- `ore` is kept for what it is actually good for: metal, with a smelting
-- ladder and a skill gate behind it. Coal has neither and should not pretend
-- to. What the finding and the working want is a different question, and it
-- now has its own column:
--
--     seam    mining this gives up something other than shards
--
-- Stated as exactly that — `not yields like '%\_shards'` — rather than as a
-- list, so a rock added later is classified by what it gives up rather than by
-- somebody remembering to add it in two places.
