...
-- the water line and no further, which is as deep as a pick works a face, and
-- a harbour was whatever depth the island had happened to leave. From a boat
-- the same shovel reaches the bottom to thirty — `dredge_depth`, off the one
-- table with `mine_depth` on it — and every spadeful deepens the water the
-- hull floats on. Nothing above the water line is dredged; that is dug, from
-- the shore.
--
-- `dredge` joins the ground family beside `dig`'s corner work: the same
-- height and soil going down together, the same rock showing when the soil is
-- gone, the same slope the digger answers for, and what comes up is what the
-- bottom is made of. The doors are the digger's, in the digger's words, with
-- the hull's in front of them: from the bank it is refused as dredging is done
-- from a boat.
