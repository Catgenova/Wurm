...
--
-- Asked from the island: clay and sand as ground you can lay. Dirt was the
-- only thing a spadeful could drop, so a clay bank or a beach could be dug up
-- and carried but never put back as itself: dropped, it raised the corner and
-- covered the ground with dirt. Now what came out of a bed goes back down as
-- what it is — dirt makes dirt, clay makes clay, sand makes sand — off one
-- table both sides read, and the menu offers the choice when more than one is
-- carried, the way it offers the four stones for slabs.
--
-- `spoil_in_hand` is the one rule for which spadeful: the item named off the
-- menu if it is one of the three, else the first of the three to hand; a
-- named item that is none of them is refused in as many words rather than
-- quietly swapped for dirt.

/** The ground each spoil makes of what it covers. */
create or replace function spoil_tile(p_item text) returns integer
  language sql stable as $fn$
  select case p_item when 'clay' then tile_id('Clay') when 'sand' then tile_id('Sand') else tile_id('Dirt') end
$fn$;

/** The spoil named off the menu, or the first of the three to hand. */
create or replace function spoil_in_hand(p_world uuid, p_uid uuid, p_item bigint) returns text
  language sql stable as $fn$
  select i.def from item i
   where i.world_id = p_world and i.holder = 'player' and i.holder_uid = p_uid and not i.locked
     and i.def in ('dirt', 'clay', 'sand') and (p_item is null or i.id = p_item)
