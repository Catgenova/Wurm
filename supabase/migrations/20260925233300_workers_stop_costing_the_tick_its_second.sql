/*
 * Workers stop costing the world tick its second.
 *
 * Asked for: "Get the world tick back under a second". The live readout had
 * it at 455 ms a round on average, 1.04 s at the 95th percentile and 2.9 s at
 * the worst, on a clock that comes round every second, with 433 ms of the
 * average in the stir -- the settling of the creatures near somebody.
 *
 * Measured here, one settle each: a wild creature 0.7 ms; a worker on a
 * settlement 5 to 17 ms; a worker in the fields 32 ms on average and 64 at
 * worst. The budget is checked between creatures, not inside one, so it is
 * the workers that carry a round past it -- and they are always near
 * somebody, because they work their keeper's settlement.
 *
 * What a worker spends it on is `find_work_tile`: every tile of its ground,
 * ring by ring, until one is work. A worker with nothing to do walks every
 * ring and finds nothing, and then it tried again four seconds later, and
 * four seconds after that, for as long as its trade stayed dry. In the fields
 * each of those tiles also brought its crop's growth up to date on its own,
 * a statement a tile, bare ground included.
 *
 *   worker_rest       how long a worker with nothing to do waits before it
 *                     looks again: four seconds at first, a second more for
 *                     every ten it has stood idle, and never more than thirty.
 *                     A trade that has run dry is looked at a fraction as
 *                     often; new work is found within half a minute.
 *   worker_settle     waits that long, and brings the growth of every crop on
 *                     a field worker's ground up to date once, in one
 *                     statement, before it searches.
 *   worker_gatherable no longer settles a crop for every tile it is asked
 *                     about.
 */
...
create or replace function public.worker_rest(p_since timestamp with time zone)
 ...
 stable
as $function$
  select make_interval(secs => least(30, 4 + extract(epoch from now() - coalesce(p_since, now())) / 10))
$function$;
...
CREATE OR REPLACE FUNCTION public.worker_gatherable(p_world uuid, p_x integer, p_y integer, p_kind text, c creature)
 ...
 STABLE
AS $function$
declare here int; t tile_def; rock rock_def; v_crop crop;
begin
  ...
  if p_kind = 'woodcut' then
    ...
    -- Growth is brought up to date for the whole search before it starts, in
    -- `worker_settle`: once, rather than once for each tile looked at. And
    -- the crop is read once here rather than three times over below, since
    -- this is asked of every tile of the ground, and most have no crop at all.
    select * into v_crop from crop cr where cr.world_id = p_world and cr.x = p_x and cr.y = p_y;
    if found and (v_crop.stage >= crop_ripe() or not v_crop.tended_now) then return true; end if;
    /*
     ...
     */
    if v_crop.x is not null then
      return exists (select 1 from item i join crop_def sd on sd.seed = i.def
                      where i.world_id = p_world and i.holder = 'ground'
                        and i.gx = p_x and i.gy = p_y);
    end if;
    -- `here` is this tile's kind, read at the top.
    return here = tile_id('Field') and (sow_seed(p_world, c, p_x, p_y)).id is not null;
