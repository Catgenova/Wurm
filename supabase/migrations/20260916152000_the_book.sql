-- A browser that has just opened holds no book of skills.
--
-- Reported from the island with a picture: an iron vein, "Iron vein needs
-- mining 5 to work. Yours is 1.0", and two lines above it in the same window
-- the island's own "Mining increased by 0.8243 to 13.2624".
--
-- `rpc_settle` learned to send the skill book only when a skill had moved,
-- counted off `player.skills_at` against `player.skills_seen`. Both are on the
-- player row. A page reload starts a new browser holding nothing but the
-- starting value of every skill, and the row it asks remembers that the *last*
-- browser was told — so it is told nothing, and sits on a book of ones.
--
-- Measured before it was touched, on a body whose island says mining is 13.26:
--
--     first beat:  mining NOT SENT — so the browser keeps its starting 1.0
--     and again:   mining NOT SENT
--     then they swing a pick once, which moves the book:
--     next beat:   mining 13.79
--
-- It only comes right when something raises a skill, and it can lock: the vein
-- that refuses you is the vein that would have raised the skill that says it
-- should not.
--
-- The same mistake as `p_said` starting at nought, and the same fix. A cursor
-- kept against the player cannot tell "you have this already" from "you have
-- just arrived"; only the browser knows which, so the browser says. It asks
-- once, gets the lot, and every beat after that is the delta.
--
-- The three-argument signature is dropped in the same breath. `create or
-- replace` with a new argument list makes a *second* function, and PostgREST
-- picks by the arguments it was handed: two candidates is every beat on the
-- island coming back "could not choose a best candidate function". `p_book`
-- defaults false, so a page that has not been redeployed resolves to this one
-- and gets exactly what it got before.

drop function if exists rpc_settle(bigint, uuid, bigint);
