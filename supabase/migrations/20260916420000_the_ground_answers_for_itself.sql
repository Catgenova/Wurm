-- Four things reported from the island at once, and three of them one fault.
--
--   * *"wildermon need to feed themselves from eligible food in deed
--     containers, they currently are not"*
--   * *"flattening should give Digging skill and gives nothing"*
--   * *"Digging and flattening clay, sand, etc tiles need to give that
--     resource not dirt"*
--   * *"just like we allowed mining up to -10 elevation, allow
--     digging/flattening up to -10 elevation"*
--
-- ## A penned animal starved beside its dinner
--
-- A wild wildermon feeds itself: `creature_settle` tops the belly up on any
-- leg that ends on ground worth grazing. One kept inside a border has no bed
-- to walk to and, until now, nothing else either — it starved standing next to
-- a crate of the very thing it eats.
--
-- So a hungry one on a deed helps itself, from a crate inside the deed's own
-- square, of something `species_diet` says its kind eats. The plainest thing in
-- there first, because the good stuff keeps for people.
--
-- ## Flattening: no skill, and the wrong spoil
--
-- `perform_ground`'s flatten branch never called `skill_raise` at all, so an
-- hour of levelling ground taught nothing. And it gave `'dirt'` whatever it was
-- scraping — dig a clay bank and you get clay, flatten the same bank and you
-- got dirt, off the same soil with the same shovel.
--
-- Both are the tile's own answer: `d.skill` off `action_def`, which the
-- `skill_said` line at the bottom of this very function has always read, and
-- `t.dig_yield` off `tile_def`, which `act_perform`'s dig branch has always
-- read. Neither number is new. Both were simply not being asked for here.
--
-- And the fill takes the tile's own stuff first and dirt after, because it
-- would be a strange rule that let you take clay out of a bank and not put it
-- back.
--
-- ## The waterline, twice
--
-- Mining was fixed to work in shallows, with the reasoning written out in
-- `terrain_refusal`:
--
--     it was a height out even where the two agree: `<= 0` refuses a face
--     standing *at* the waterline, while water is only drawn below it
--
-- Digging kept the exact line that comment condemns — `land_height(cx, cy) <=
-- 0` — and flattening kept a blanket `has_water`, which refuses a tile with a
-- single corner an inch under. Both read `mine_depth()` now, which is the one
-- statement of how deep any ground work may be done in.

/**
 * A hungry wildermon on a deed helps itself from the stores.
 *
 * The crate has to be inside the square of a deed that also covers the
 * animal — `deed_covering` and `deed_covers` are the one statement of what a
 * settlement reaches, so this asks the same question the builder and the
 * treasure roll ask. Nothing wild, and nothing off somebody else's land.
 */
create or replace function worker_feed(p_world uuid, p_id integer) returns text
...
begin
  ...
  -- The plainest thing it eats, out of any crate on that land.
  select i.* into v_it
    from crate k
