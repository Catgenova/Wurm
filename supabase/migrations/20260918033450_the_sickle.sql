...
--
-- Asked from the island. Prune went in with the hatchet because the island
-- had no sickle, and said so. It has one now: a blade cast at the anvil from
-- one lump, in a mould fired from sand like every other, fitted to a handle
-- with two nails like the hatchet, and a smith's to improve. Prune is the
-- sickle's from here — the action's own row says so, so the family's "you
-- need a sickle" comes for nothing — and the work wears the sickle rather
-- than the hatchet.
--
-- And it cuts what a bush has on it: rose petals off a rose bush, lavender
-- off a lavender bush, each a dyestuff the dyer's table already knows how to
-- boil, and nothing off a thorn bush that is worth the blade. `bush_def.yields`
-- says which, off the same table the browser reads. Cut, a bush is picked
-- clean for a while, on the same clock as fruit off a tree.
...
AS $function$
  select p_action in ('flatten', 'drop_dirt', 'pave_slabs', 'remove_paving',
                      ...
                      'plant', 'prune', 'harvest_bush', 'dig_stump', 'dig_worms', 'prospect')
$function$
;
