import{Lr as e,M as t,N as n,Ot as r,Qi as i,Qn as a,Ui as o,Wi as s,t as c}from"./boards.js";var l=()=>{let t=e(a).bill.map(([e,t])=>`<b>${t} × ${i(e).name.toLowerCase()}</b>`);return`${t.slice(0,-1).join(`, `)} and ${t[t.length-1]}`};function u(){let e=l();return`
    ...
    every 30 seconds while it is open. <b>Skills:</b> ${c.skills}
    <b>Wildermon:</b> ${c.wildermon} <b>Settlements:</b> ${c.settlements}
    ...
    and otherwise at the knife's QL times ${t.toFixed(1)} to ${(t+n).toFixed(1)}, never above
    your Butchering; with bare hands it comes off at your Butchering times ${t.toFixed(1)} to
    ${(t+n).toFixed(1)}, plus 1. The menu's <b>Butcher</b> line says the range for the knife you carry.
    ...
    ${(s*100).toFixed(0)}% a step &mdash; ${[1,2,3].map(e=>`<b>${(s*e*100).toFixed(0)}%</b>`).join(`, `)}
    ...
    with a mallet from ${e}; it weighs <b>${i(a).weight} kg</b> and does not rot.
