import{$ as e,$n as t,$o as n,$s as r,$t as i,Aa as a,Ai as o,At as s,Co as ee,Ct as c,Di as te,Do as ne,Dt as re,Ei as ie,Et as l,Fi as u,Fn as ae,Fo as oe,Fr as se,Ft as ce,G as le,Gn as ue,Go as d,Gs as de,Hi as fe,Ho as pe,Hs as me,Ht as he,Ii as ge,Ir as f,Is as _e,Ji as ve,Jo as ye,Js as p,K as be,Ka as xe,Ks as m,Mi as Se,Mn as Ce,Mo as we,No as Te,Nr as Ee,Nt as De,Oi as h,On as Oe,Oo as g,Ot as ke,P as Ae,Po as _,Pr as je,Pt as Me,Q as v,Qn as Ne,Qo as Pe,Qs as y,Qt as Fe,Rr as Ie,Rs as Le,So as Re,Ss as ze,St as Be,Ti as Ve,To as b,Tr as He,Ts as Ue,Tt as We,U as x,Ua as Ge,Uo as S,Vt as Ke,W as qe,Ws as C,X as Je,Xo as Ye,Xs as w,Xt as Xe,Ya as T,Yr as Ze,Ys as E,Yt as Qe,Z as $e,Za as D,Zs as O,_a as et,_c as tt,_i as nt,_n as rt,_t as it,a as at,aa as ot,ac as st,as as ct,bo as lt,bt as ut,ca as dt,ci as ft,co as pt,cs as mt,ct as ht,d as gt,di as _t,dr as vt,ds as yt,ec as bt,en as xt,er as St,es as Ct,et as wt,fa as Tt,fi as Et,fn as Dt,fr as Ot,fs as kt,g as At,gi as jt,go as k,gt as Mt,h as Nt,ha as Pt,hc as Ft,ho as It,ht as Lt,ia as Rt,ii as zt,is as Bt,it as A,ji as Vt,jn as Ht,jo as Ut,jt as Wt,ki as Gt,kt as Kt,la as qt,li as Jt,ln as Yt,lo as j,mc as Xt,mi as Zt,mn as Qt,mo as M,mt as $t,nc as en,ni as tn,ns as nn,nt as rn,oa as an,oo as on,os as sn,pi as cn,pn as ln,ps as un,q as dn,qa as fn,qi as pn,qn as mn,qs as N,ra as hn,ri as gn,rs as _n,rt as vn,sa as yn,so as P,t as bn,tn as xn,ts as Sn,tt as Cn,ua as wn,ui as Tn,un as En,us as Dn,v as On,vi as kn,vr as An,vt as jn,wa as Mn,wi as Nn,wr as Pn,wt as Fn,xi as In,xo as Ln,xt as Rn,ys as zn,yt as Bn,zn as Vn}from"./boards.js";import{B as Hn,F as Un,G as F,H as Wn,R as Gn,Y as Kn}from"./supabase.js";var qn=()=>`${+(450/60).toFixed(1)} minutes`,I=e=>{let t=He.get(e);if(!t)throw Error(`The help names a recipe there is not: ${e}`);return t},L=(e,t=!1)=>pe(I(e).inputs.map(e=>[e.item,e.count??1]),t),R=(e,t)=>{let n=I(e).inputs.find(e=>e.item===t);if(!n)throw Error(`The help says ${e} takes ${t}, and it does not.`);return n.count??1},z=e=>I(e).count??1,Jn=e=>{let t=I(e);return`${(Ft.get(t.skill)?.name??t.skill).toLowerCase()} ${t.difficulty??0}`},B=e=>{let t=me.get(e);if(!t)throw Error(`The help names a material there is not: ${e}`);return t},V=e=>Hn[e],H=e=>1-Kn(V(e).roll,1),U=(e,t)=>d(e).feeds?.[t]??0,W=100,G=1,Yn=3600,Xn=e=>`${String(Math.floor(e)).padStart(2,`0`)}:${String(Math.round(e%1*60)).padStart(2,`0`)}`,Zn=Object.values(A).filter(e=>e.glow),Qn=Object.values(A).filter(e=>e.wound===`burn`),$n=18/24,K=e=>Ft.get(e)?.start??1,q=100,er=100,tr=P.plate.burden*B(`iron`).weight,nr=e=>Math.round(100/(De(e)*B(`copper`).wear)/10)*10,rr=80,ir=90,ar=e=>Math.ceil(rr/ae(e).healed),or=e=>`${+(rr*ae(e).cost).toFixed(1)} points`,sr=Object.values(sn),cr=`${c} to ${+(c+Fn).toFixed(2)} times`,J=_.map((e,t)=>t).slice(1),lr=ne.get(`study_book`)?.baseTime??0,ur=[...Le].sort((e,t)=>e.difficulty-t.difficulty),dr=[...new Set(on.map(e=>e.slot))],fr=Object.values(P).filter(e=>!on.some(t=>t.cls===e.id&&He.get(`make_${t.id}`)?.inputs.some(e=>e.item===`dragon_scale`))),pr=[...new Set(j.map(e=>e.kind))],mr=j.filter(e=>e.ammo&&He.get(`make_${e.id}`)?.inputs.some(e=>e.item===`bow_string`)),hr=Object.values(A).filter(e=>e.butcher?.feather||e.shearYield===`feather`),Y=g[g.length-1],gr=e=>bt.find(t=>t.id===e)?.level??0,_r=A.orse.speed*re(0),vr=Math.min(5,A.orse.speed*re(q)),yr=N(Object.entries(mn).map(([e,t],n)=>`level ${e} ${n===0?`wants `:``}${N(t.map(e=>`${e.label[0].toLowerCase()}${e.label.slice(1)}`))}`)),br=[...new Set(Object.values(A).filter(e=>!e.monster).map(e=>e.workRange))].sort((e,t)=>e-t),xr=(()=>{let e=new Map;for(let t of Object.values(A))t.rangePerStep&&t.rangePerStep!==10&&e.set(t.rangePerStep,[...e.get(t.rangePerStep)??[],`a ${t.name}`]);return N([...e.entries()].sort((e,t)=>e[0]-t[0]).map(([e,t])=>`${N(t).replace(/ and ([^,]*)$/,` or $1`)} ${e}`))})(),Sr=N([0,20,40,q].map((e,t)=>t===0?`${p(et(e))} pass below ${p(20)}`:`${p(et(e))} at ${p(e)}`)),Cr=e=>e&&`${e[0].toLowerCase()}${e.slice(1)}`,wr=Object.values(A).filter(e=>e.monster),Tr=wr.map(e=>e.notice??7).sort((e,t)=>e-t),Er=v.reduce((e,[,t])=>e+t,0),Dr=we.filter(e=>!r.some(t=>e===`${t}_lump`)),Or=j.find(e=>e.id===`composite_bow`)??j[0],kr=[...mr].sort((e,t)=>(t.range??0)-(e.range??0))[0]??j[0],Ar=on.filter(e=>e.cls===`scale`).reduce((e,t)=>e+R(`make_${t.id}`,`dragon_scale`),0),jr=Object.fromEntries(h.map(e=>{let t=Gt.filter(t=>t.tier===e).map(e=>Math.max(...Object.values(e.effects).map(e=>Math.abs((e??1)-1))));return[e,[Math.min(...t),Math.max(...t)]]})),X=Un.filter(e=>e.level).sort((e,t)=>(e.level??0)-(t.level??0)),Mr=e=>e.replace(/ (vein|seam)$/,``).toLowerCase(),Nr=[1,50,90,99],Pr=e=>tt(e,Be,1),Fr=e=>String(Number(Pr(e).toPrecision(Pr(e)>=.1?2:1))),Ir=()=>{let e=a(Ze).bill.map(([e,t])=>`<b>${t} × ${d(e).name.toLowerCase()}</b>`);return`${e.slice(0,-1).join(`, `)} and ${e[e.length-1]}`},Lr=ot[Rt[0].id].length,Rr=yn/Lr,zr=e=>an.find(t=>t.rank===e)?.cost??0,Br=e=>zr(e)===1?`a point`:`${p(zr(e))} points`,Vr=(e,t)=>an.find(n=>n.channel===e&&n.rank===t)?.note??``,Hr=[...new Set(dt.map(e=>Object.keys(e.muls).length))].sort((e,t)=>e-t),Ur=e=>Object.entries(wn(e)?.muls??{}).map(([e,t])=>`${hn[e].note.toLowerCase()} ${qt(t)}`).join(`, `),Wr=Ft.get(`awareness`)?.start??20,Gr=Object.keys(Te).filter(e=>Te[e].holds),Kr=Mn.filter(e=>He.get(`make_${e.id}`)?.skill===`fine_carpentry`),qr=Kr.filter(e=>e.capacity&&!e.liquid).sort((e,t)=>(e.capacity??0)-(t.capacity??0)),Jr=Object.keys(ze).length,Yr=Mn.filter(e=>e.boat),Xr=a(`rowing_boat`),Z=a(`sailing_boat`),Q=a(`caravel`),Zr=a(`large_cart`),$=a(`wagon`),Qr=Mn.filter(e=>e.liquid).sort((e,t)=>(e.liquid??0)-(t.liquid??0)),$r=a(`craft_bin`).heft??0,ei=a(`seed_bin`).heft??0,ti=a(`sprout_bin`).heft??0,ni=11,ri=(at.Gold??0)/(at.Silver??1),ii=5,ai=[...st].filter(e=>!r.includes(e)),oi=40,si=70,ci=ve.get(`cider`)??pn[0],li=Wn.filter(e=>e.fruit),ui=Ln.flatMap(e=>[b(e,1),b(e,W)]),di=Math.min(...ui),fi=Math.max(...ui),pi=e=>{let t=en.find(t=>t.id===e);if(!t)throw Error(`The help names a mould there is not: ${e}`);return`poured at the smelter and beaten out on an anvil, ${t.lumps===1?`a lump of metal`:`${p(t.lumps)} lumps of metal`} ${t.per?`to ${p(t.per)}`:`to each`}`},mi=e=>{let t=e=>e.speed*Kn(e.roll,1);return 60*t(V(F.Slabs))/t(V(e))},hi={ql:50,force:.6},gi=N([Math.PI/2,Math.PI,Me,0].map(e=>(Z.boat?Kt(Z.boat,hi.ql,20,he(hi.force)*ce(e),0):0).toFixed(1))),_i=()=>{let e=Object.values(o).map(e=>`<b>${e.label}</b> (${e.means})`);return`${e.slice(0,-1).join(`, `)} or ${e[e.length-1]}`};function vi(){let r=Ir(),o=_i();return`
    ...
    under ${E(At)} of the health it can carry, a <b>hungry</b> one on anything under ${E(Nt)} fed, and the card
    ...
    ${qn()}, the island counts you as gone, and from then until you come back it keeps count for
    ...
    &mdash; <kbd>1</kbd> to <kbd>9</kbd> and <kbd>0</kbd> do the <b>first ${p(On.length)} things that can actually
    ...
    the handful as well as one at a time: <b>once</b>, ${gt.map(p).join(`, `)}, or <b>until you
    ...
    read a dark hillside, so it <b>starts at ${Wr}</b> where the others start at 20 &mdash; and the
    ${p(15)} tiles on the flat that this island used to give everybody is what it gives somebody at the
    top of it. At ${Wr} you see ${E(xn(Wr))} of that &mdash; ${p(Math.round(15*xn(Wr)))}
    ...
    is ${p(Math.ceil(1/xt))} trees deep to the eye, and the <b>dark takes ${w(i)} of everything</b>. Your
    ...
    <p>Each trade earns <b>points</b> off the best skill it covers, and its tree is <b>${p(Lr)} columns of
    ${p(Rr)}</b>. A column is one <b>channel</b>: one number in the rules, and every card names it and says
    exactly what that node does to it &mdash; ${[`hands`,`aim`,`knit`].map((e,t)=>`<i>${Vr(e,[1,1,Rr][t])}</i>`).join(`, `)}.
    The lower nodes cost ${Br(1)} each, the one above them costs ${p(zr(Rr))}, and the lower must be
    bought first. A few channels are better lower &mdash; ${N(Object.values(hn).filter(e=>e.lower).map(e=>e.note.toLowerCase()))}
    &mdash; and the sign on the card is the change to the number, so those read as a minus.</p>
    <p>A <b>rite</b> is the one thing a trade may ask for out loud: ${N(Hr.map(p)).replace(` and `,` or `)} channels pushed much
    ...
    for another &mdash; ${wn(`redhour`)?.name} is <i>${Ur(`redhour`)}</i> &mdash; so read both halves before you call one.
    Only the <b>fighting</b> trades have a rite; a craft trade has its ${p(yn)} nodes and none. It sits at
    ...
    and the well, and <i>nail</i> finds all ${p(Pn.filter(e=>e.inputs.some(e=>e.item===`nail`)).length)} recipes that want nails. The count at the foot
    ...
    (<kbd>J</kbd>) is that list: ${p(tn.length)} goals in ${p(gn.length)} chapters, from felling your first tree to
    ...
    <p><b>The ${p(Wn.filter(e=>!e.fruit).length)} woods that bear nothing.</b> <b>Pine</b> is soft, light and quick to work, and rots as fast as it grew.
    <b>Willow</b> and <b>birch</b> are light and springy. <b>Maple</b> is even-tempered. <b>Oak</b> is
    hard going and worth it: an oak thing takes ${E(B(`oak`).wear/B(`pine`).wear)} of the knocks a pine one does, holds more
    ...
    <p><b>The ${p(bt.length)} metals.</b> <b>Copper</b> is what everything starts in and is soft with it.
    ...
    which is iron with coal beaten through it, keeps an edge ${y(B(`copper`).wear/B(`steel`).wear)} as long as copper and does not
    mind the weather. <b>Bronze</b> and <b>brass</b> are the first alloys worth a crucible. <b>Silver</b> hardly
    tarnishes and bites anything that carries its own light ${y(pt)} as hard, which is what a
    <b>Lume</b> or an <b>Embra</b> is. <b>Gold</b> rots at ${E(B(`gold`).decay/B(`copper`).decay)} of copper's pace, weighs
    ${y(B(`gold`).weight/B(`copper`).weight)} what copper does and is good for nothing else. The ${p(ai.length)} out of the deep seams are what a
    lifetime of mining is for: <b>adamantine</b> takes the keenest edge, <b>glimmersteel</b> is light and turns
    aside ${E(B(`glimmersteel`).soak/B(`copper`).soak-1)} more than copper does, <b>mithril</b> is lighter than the wood it is hafted to, and
    <b>seryll</b> scarcely takes a mark at all. A tool's metal decides how fast and how true it works, so a
    bronze hatchet at ${oi} works as well as a copper one at ${Math.round(oi*B(`bronze`).bite/B(`copper`).bite)}; a weapon's metal decides
    what it does; armour's metal decides both what it stops and what it costs you to carry.</p>
    <h3>Brewing</h3>
    <p>Fill a barrel from a well or the shore, stand at it and <b>set a brew going</b>. ${C(pn.length)} of them:
    ${N(pn.map(e=>`<b>${e.name.toLowerCase()}</b> from ${S(e.input,e.count,!0)} and ${e.litres} litres of water in ${O(e.time)}`))}.
    Each gives back as many litres of drink as it took of water.</p>
    <p>A <b>quern</b> presses fruit too: ${p(R(`press_apple_juice`,`apple`))} of any fruit into a <b>bucket of juice</b>, sweet and with
    nothing dangerous in it, and ${p(R(`press_apple_cider`,`apple`))} apples or pears straight into a <b>bucket of cider</b> with no
    barrel and no waiting &mdash; one bucket, where the barrel would have made ${p(ci.litres/5)} from ${S(ci.input,ci.count)}
    and ${p(ci.litres)} litres of water.</p>
    ...
    and a brew carries it far longer than food &mdash; at quality ${si}, a baked potato's lasts
    ${O(b(`baked_potato`,si))} and a bucket of wine's ${O(b(`wine_bucket`,si))}. A barrel of the right thing
    ...
    <p>${C(u.length)} fish run at ${p(new Set(u.map(e=>e.depth)).size)} depths, and each wants a hand to match:
    ${N(u.map(e=>`<b>${e.name.toLowerCase()}</b> ${e.depth>0?`from ${p(e.depth)} deep`:`anywhere there is water`}${e.level>u[0].level?` with fishing ${e.level}`:``}`))}.
    ...
    what comes off it follows the size of the fish &mdash; a pike is ${p(z(`cook_pike`))} helpings and a sturgeon
    ${p(z(`cook_sturgeon`))}.</p>
    ...
    all of it, a sturgeon is ${E(ge(`sturgeon`))} of what takes a bare hook and ${E(ge(`sturgeon`,`perch`))} of what
    takes a perch.</p>
    ...
    are elsewhere, holds ${p(f.creel.hold??0)}, and gives about <b>${p(Math.round(1/Ee))} fish to a baiting</b> before the bait is
    worked out of it. Empty it from the bank. Kept baited and emptied, the best one that can be woven is
    worth around ${p(Math.round(Yn/45*Ie(W)))} fish an hour for no work at all.</p>
    ...
    <h3>Fruit trees</h3>
    <p>${C(li.length)} of the ${p(Wn.length)} trees bear. <b>Apple</b> and <b>olive</b> grow wild here and there in
    ...
    minutes before there is anything on it again. ${m(S(`apple`,R(`make_apple_pie`,`apple`)))} and a dough bake into ${p(z(`make_apple_pie`))} <b>apple
    pies</b>, the best food on the island; ${S(`cherry`,R(`make_cherry_preserves`,`cherry`))} boil down into ${p(z(`make_cherry_preserves`))} jars of <b>preserves</b>;
    and ${S(`olive`,R(`press_olives`,`olive`))} crushed under a <b>quern</b> give ${p(z(`press_olives`))} measures of <b>olive oil</b>, which keeps almost
    for ever.</p>
    ...
    other &mdash; and a field's menu offers to <b>Sow</b> whichever you carry. A crop goes through ${p(fn.length)}
    stages: ${N(fn)}, each drawn differently, and every crop takes its own time
    ...
    fire is the place to <b>Cook</b>: raw meat becomes cooked meat that fills ${E(d(`cooked_meat`).food??0)} of the food bar where
    raw fills ${E(d(`meat`).food??0)}, potatoes bake
    ...
    <b>Milling</b> skill. ${m(S(`wheat`,R(`make_flour`,`wheat`)))} grind down to a lot of <b>flour</b>, ${S(`corn`,R(`make_cornmeal`,`corn`))} to <b>cornmeal</b>,
    and a badly ground batch is nothing but grit.</p>
    <p>Flour and a bucket of water are worked into <b>dough</b> &mdash; ${p(z(`make_dough`))} rounds at a time, and the
    ...
    ${p(z(`make_porridge`))} bowls of <b>porridge</b>. Both are cooking rather than milling: the mill only makes the meal.</p>
    <h3>The oven</h3>
    ...
    ${p(a(`oven`).w*a(`oven`).h)} spots. Feed it anything a fire takes, peat and coal included &mdash; it holds ${O(fe)} of
    ...
    and otherwise at the knife's QL times ${c.toFixed(1)} to ${(c+Fn).toFixed(1)}, never above
    your Butchering; with bare hands it comes off at your Butchering times ${c.toFixed(1)} to
    ${(c+Fn).toFixed(1)}, plus 1. The menu's <b>Butcher</b> line says the range for the knife you carry.
    ...
    found a settlement: ${ni} by ${ni} tiles
    around a stone token. You may hold one settlement at a time, and building is only allowed
    on its land. Things left outside on deed land rot ${y(1/Ut)} slower.</p>
    ...
    on packed ground or in the long grass &mdash; and choose <b>Plan fence</b>. A log fence is ${p(_e(`log`,`fence`).needed.log)} logs
    where a log wall is ${p(_e(`log`,`solid`).needed.log)}; a half wall is ${w(Ue.get(`half_wall`)?.factor??0)} of one. Both stop anything alive at that border,
    ...
    poured into it there and beaten out on an anvil gives ${p(5)} nails at ${p(Math.round(d(`nail`).weight*1e3))} grams apiece. Crates, tool heads fitted to their
    ...
    a mallet, planks, timbers, shafts and nails it builds ${p(Kr.length)} pieces &mdash;
    ${N(Kr.map(e=>e.name.toLowerCase()))}. Each is carried like a crate and
    <b>set down</b> on a block of subtiles: right-click a tile and choose <b>Set furniture down</b>, and
    the piece follows the cursor until you click it down &mdash; <b>Q</b> and <b>E</b> turn it ${w(1/Jr)}, Escape
    keeps it. A piece stands the way it was set however the view is turned, and <b>Turn it</b> on a standing
    piece turns it ${w(1/Jr)} round. Staircases and ladders are planned the same way, Q and E
    choosing the side you climb from.</p>
    <p>${C(qr.length)} of them hold things: ${N(qr.map((e,t)=>`${e.name.endsWith(`s`)?``:`a `}${e.name.toLowerCase()} ${t===0?`takes `:``}${e.capacity}`))},
    where a plank crate takes ${D.plank.capacity}. The <b>larder</b> is the only one of them that is fussy: it takes food and drink, raw
    ...
    a road wants.</p>
    <h3>Bags</h3>
    <p>${C(Gr.length)} things hold other things and are carried in your pack:
    ${N(Gr.map(e=>`a <b>${d(e).name.toLowerCase()}</b> of ${L(`make_${e}`,!0)} (${d(e).holds} things)`))}.
    ...
    ${N(Gr.map((e,t)=>`${w(d(e).shelter??1)}${t===0?` the rate`:``} in a ${d(e).name.toLowerCase()}`))}.
    ...
    short rein &mdash; <b>${An(1)} tiles round a rough post and ${An(W)} round the best</b>, however much the creature
    ...
    all the way home.</p>
    <p>Nothing holds a post up and it <b>rots where it stands</b>: <b>${O(Ot)}</b> for the
    roughest and <b>${O(vt)}</b> for the best that can be made, leaning further as it goes, with one
    ...
    <p>It is the first of the ${p(Mn.filter(e=>e.heft).length)} stores that <b>do not count what is in them</b>. It weighs it:
    <b>${$r} kg</b>, which is ${p(Math.round($r/d(`nail`).weight))} nails or ${p(Math.round($r/d(`plank`).weight))} planks, and there is no
    ...
    <p>A <b>seed bin</b> and a <b>sprout bin</b> are the small pair, ${p(a(`seed_bin`).w*a(`seed_bin`).h)} subtile each, ${L(`make_seed_bin`)}
    apiece. Both weigh what is in them rather than counting it: the seed bin <b>${ei} kg</b>, which is
    ${Math.round(ei/d(`wheat_seed`).weight)} wheat seeds or ${Math.round(ei/d(`potato_seed`).weight)} seed potatoes, and the sprout bin <b>${ti} kg</b>, which is
    ${Math.round(ti/d(`sprout`).weight)} sprouts. The seed bin takes the
    ${p(xe.size)} sowable seeds and nothing else; the sprout bin takes sprouts and nothing else.</p>
    <p>A <b>trash crate</b> is built with a rotten bottom on purpose: anything put in it rots <b>${y(a(`trash_crate`).trash??1)}
    ...
    least ${p(3)} units below the deck &mdash; a gap, not a slope. ${C(Object.keys(Vn).length)} kinds:
    ${N(Object.values(Vn).map(e=>`a <b>${e.name.toLowerCase()}</b> (${pe(e.bill)} a span${e.tool===`trowel`?`, with a trowel`:``}) goes <b>${p(e.span)}</b> tiles and ${e.carts?`carries a cart`:`takes foot traffic only`}`))}.</p>
    ...
    <h3>Boats</h3>
    <p>${C(Yr.length)} hulls, all of them a carpenter's work. A <b>rowing boat</b> is ${L(`make_rowing_boat`,!0)};
    she carries <b>${Xr.capacity} things</b>, wants <b>${p(Xr.boat?.draught??0)} deep</b> of water under her and is rowed, so your
    <b>body strength</b> is the engine. A <b>sailing boat</b> is ${L(`make_sailing_boat`,!0)};
    she carries <b>${Z.capacity}</b>, wants <b>${p(Z.boat?.draught??0)} deep</b>, and the wind
    ...
    <b>${p(Q.boat?.draught??0)} deep</b>, makes <b>${Q.boat?.speed}</b> tiles a second at a fair effort to the
    sailing boat's <b>${Z.boat?.speed}</b>, and carries <b>${p(Q.boat?.passengers??0)} passengers</b> besides
    ...
    ${p(Q.boat?.passengers??0)} places on deck that is free, and from then on you go where she goes and your own
    ...
    steady and slower; hard up into it is hard work; and inside the last ${p(Math.round(Me*180/Math.PI))} degrees she is
    ...
    and do the same on the other. A sailing boat of quality ${hi.ql} with body control 20 at the tiller, in
    ${Ke(hi.force)}, makes ${gi} tiles a second on a beam reach, running, close-hauled and in irons.</p>
    ...
    <p>A sailing boat <b>holds ${p(Z.capacity??0)} things</b>, crates included, and that is what she is for
    &mdash; but a hull loaded to her marks is ${E(it)} slower than one running empty.</p>
    <h3>Why a road is worth its stone</h3>
    <p>Feet hardly care what is under them: sand is walked at ${E(V(F.Sand).speed)} of the pace of grass and laid
    stone at ${E(V(F.Slabs).speed)}, and only a bog really tells, at ${E(V(F.Marsh).speed)}. A <b>laden wheel</b> cares about very
    ...
    <p>Stone slabs and cobble cost a full wagon <b>nothing</b>. Packed dirt costs ${w(H(F.PackedDirt))}, bare
    grass ${w(H(F.Grass))}, sand ${w(H(F.Sand))}, a tilled field ${w(H(F.Field))}, and a <b>bog ${w(H(F.Marsh))}</b>. What a full
    wagon crosses in a minute on a paved road takes it <b>${O(mi(F.Grass))} over grass and
    ${O(mi(F.Marsh))} through marsh</b> &mdash; which is the whole argument for paving, and why the stone is
    worth cutting.</p>
    ...
    <ul>
      ...
      <li><b>Big axle</b> &mdash; an <b>axle mould</b> ${pi(`axle_mould`)}.</li>
      <li><b>Metal ribbon</b> &mdash; a <b>ribbon mould</b> ${pi(`ribbon_mould`)}.</li>
      <li><b>Yoke</b> &mdash; ${L(`make_yoke`)}, stitched with an awl. One per hitch.</li>
    </ul>
    <p>A <b>large cart</b> takes ${L(`make_large_cart`,!0)}. It holds <b>${Zr.capacity} things of any weight</b> and has ${p(Zr.vehicle?.yokes??0)} yokes:
    ${p(Zr.vehicle?.needs??0)} wildermon will move it, ${p(Zr.vehicle?.yokes??0)} move it faster.</p>
    <p>A <b>wagon</b> takes ${L(`make_wagon`,!0)}. It holds <b>${$.capacity} things</b> and will not stir until
    <b>${$.vehicle?Ge($.vehicle):``} yokes</b> have a wildermon in them.</p>
    ...
    drags its feet, so feed the team before it goes in. A Seavic pair will outrun you at a walk; ${p($.vehicle?.needs??0)}
    Quarra will not, but they will shift ${p($.capacity??0)} bricks.</p>
    ...
    <p><b>Barrels</b> hold liquid and nothing else, in ${p(Qr.length)} sizes:
    ${N(Qr.map((e,t)=>`<b>${e.name.toLowerCase()}</b> (${e.liquid}${t===0?` litres`:``})`))}. One barrel holds one liquid &mdash; water or lye, not
    ...
    <p>Sleeping in a bed banks <b>rest</b> &mdash; ${w(ee)} the night at a perfect bed and less in a poorer one,
    up to ${O(Re)} of it held at a time. Rest burns only while you are actually working, and everything you
    do while it burns <b>teaches you ${y(2)} as much</b>. The hud shows how much you have left.</p>
    <p>Every cooked dish <b>favours one trade</b>, and eating it leaves a <b>knack</b> for that trade
    for a while &mdash; ${y(1+lt)} as much, for anything from ${O(di)} to ${O(fi)} by how
    ...
    an afternoon of carpentry that teaches you ${y(1+lt)} as much.</p>
    <h3>What is actually in a meal</h3>
    <p>Filling the food bar takes a raw potato. Eating <i>well</i> is a different question. ${C(ln.length)} things
    a body wants are kept separately under the food bar, each fed by different food and each falling
    away on its own over <b>${O(Qt)}</b>:</p>
    <table>
      <tr><td><b>Starch</b></td><td>bread, porridge, roots and grain</td></tr>
      <tr><td><b>Flesh</b></td><td>meat and fish &mdash; cooked, meat is worth ${y(U(`cooked_meat`,`flesh`)/U(`meat`,`flesh`))} raw</td></tr>
    ...
    <p>Raw food feeds one of them a little: a potato is ${E(U(`potato`,`starch`))} starch, a piece of
    meat ${E(U(`meat`,`flesh`))} flesh. A <b>cooked dish feeds several, and feeds them properly</b> &mdash; bread is
    ${E(U(`bread`,`starch`))} starch, cooked fish ${E(U(`cooked_fish`,`flesh`))} flesh and ${E(U(`cooked_fish`,`fat`))} fat, and a <b>stew</b> is the only thing on
    the island that feeds all ${p(ln.length)} at once. Better cooking fills them fuller, so quality tells here as
    everywhere.</p>
    <p>What comes of it: anything in you at all <b>holds hunger and thirst off</b>. On a full board
    they fall at <b>${w(1-Dt)}</b> of their ordinary pace, which is ${O(.5/(Oe*(1-Dt)))} rather
    than ${O(.5/Oe)} before the hunger bar is down to half. And a board with <b>all of them</b> full makes
    everything you do teach you <b>${w(rt)} more</b> &mdash; but that one reads off whichever of them
    ...
    now they have bought nothing at all. One number settles it: <b>a gold coin is worth ${p(ri)}
    silver</b>, every price is named in silver, and change comes back in silver. Paying takes your
    largest coins first, so ${p(ri+1)} silver out of a gold and ${p(ii)} leaves you the ${p(4)} rather than
    ...
    still holds comes back to your purse; one left open for <b>${O(Ae)}</b> lapses and gives
    ...
    up the gardens, empty the stores and pull the walls down. There are ${p(ut.length)} standings now.</p>
    <table>
    ...
    <p>The <b>Field guide</b> window (<kbd>F4</kbd>) has a page for each of the ${p(Qe().length)} kinds of
    ...
    all ${p(Qe().length)}, tamed and bred out of the ${p(Qe().filter(Xe).length)} that
    ...
    worth. The copper chisel you washed ashore with is quality ${p(s(`chisel`))}, so it reaches your ceiling
    about one go in ${p(Math.round(100/s(`chisel`)))}. That is the whole reason to better a tool, and every recipe row in
    ...
    <p><b>Everything rots where it lies.</b> A deed slows that to ${w(Ut)}, a roof over a closed
    room slows it to ${w(zn)} again, and a crate or a sack slows it further still. A pile of planks
    ...
    ${D.plank.capacity} things of every sort the deed turns up, so it is the store that needed it most.</p>
    ...
    <p>Night takes <b>${w(i)}</b> of your sight, which is enough to be a reason to stop walking.
    ${C(mt.length)} things go in your hand against it.</p>
    <p>A <b>torch</b> is ${S(`cloth`,R(`make_torch`,`cloth`))} wound round the head of ${S(`shaft`,R(`make_torch`,`shaft`))} (${Jn(`make_torch`)}) and is
    the poor relation in every way that matters: <b>${p(un(G))} to ${p(un(W))} tiles</b> of light
    for <b>${O(kt(G))} to ${O(kt(W))}</b>, by how well it was wound, and when it is
    ...
    <p>A <b>lantern</b> (${L(`make_lantern`)} &mdash; ${Jn(`make_lantern`)}) takes a <b>candle</b>
    drawn from beeswax and yarn. A better one keeps the draught off the flame and throws further:
    <b>${p(yt(G))} tiles and ${O(Dn(G))}</b> to a candle at the roughest,
    <b>${p(yt(W))} tiles and ${O(Dn(W))}</b> at the best.</p>
    ...
    <p>Carrying one lit gives back <b>${w(Fe)}</b> of what the dark takes from your sight, and its
    ...
    <b>oven, kiln or smelter</b> in blast ${p(4)}, and the ${p(Zn.length)} creatures that carry a
    light of their own as far as it reaches &mdash; ${N(Zn.map(e=>`a ${e.name.toLowerCase()} ${p(e.glow??0)}`))}.
    ...
    <b>${O(Bn)}</b> of real time, ${O(Bn/24)} to the game hour. The sun goes down at
    <b>${Xn(21)}</b> and comes up at <b>${Xn(3)}</b>, so <b>${w($n)} of it is day and
    ${w(1-$n)} night</b> &mdash; ${O(Bn*(1-$n))} of real time dark in every day
    ...
    <b>${Xn(3+We)}</b>, and the world has not waited for you: fires burn down, crops come on,
    ...
    <p><b>Pick up everything here</b> sweeps the tile you are standing on and the ${p((2*Rn.SWEEP+1)**2-1)} around it in
    ...
    strength</b>, so 220 at the start and ${120+q*5} at ${p(q)}. Going over it costs, and
    the cost climbs with how far over you are: everything past the mark slows you and every action
    takes more wind, as armour does &mdash; ${p(er)} kilos past a starting back drags on you like about
    ${p(Math.round(Wt(er,220)/tr))} full suits of iron plate. Past <b>${y(jn)}</b> your limit you are down to
    <b>${w(En)} of your pace</b>. The inventory footer turns red and says by how much, and the bars
    say so too.</p>
    ...
    is for: a copper tool is good for about ${p(nr(G))} jobs at quality ${G} and ${p(nr(W))} at
    ${p(W)}, and what it is made of stretches or shortens that. Damage also makes a tool work as though it
    ...
    Taking ${p(rr)} damage out of a tool is ${p(ar(K(`repair`)))} goes at repair ${K(`repair`)} and costs it
    ${or(K(`repair`))} of quality; at repair ${ir} it is ${p(ar(ir))} goes for ${or(ir)}.
    ...
    <p>${C(sr.length)} kinds, and each has a herb that suits it:
    ${N(sr.map((e,t)=>`a <b>${e.name}</b> ${t===0?`wants `:``}<b>${e.herb}</b>`))}. What leaves which
    is what hit you &mdash; a hoof bruises, a claw opens, a sting goes deep and narrow, and
    ${N(Qn.map(e=>`the ${e.name.toLowerCase()}`))} burn.</p>
    <p>A wound is dressed with a bandage or a cover. A <b>bandage</b> is cloth cut ${p(z(`make_bandage`))} to a length with a knife; a
    <b>healing cover</b> is the herb itself, bruised into cotton, ${p(z(`make_cover_${sr[0].herb}`))} to a batch on the
    <b>first aid</b> skill. Put on cleanly, either stops the bleeding and puts back between ${E(St(K(`first_aid`),G))} and
    ${E(St(q,W))} of your health at once, by your first aid and its quality. Right-click either
    ...
    ${y(n/Pe)} as fast as with nothing on it. The <b>right herb</b> puts back
    ${y(t)} as much at once, closes it ${y(nn/n)} as fast as cloth, and it <b>never
    turns</b>; the wrong herb puts back ${y(Ne)} as much and closes it ${E(_n/n-1)} faster than cloth.
    Cloth leaves ${E(Bt)} of an open wound's chance of going bad, the wrong herb ${E(ct)}.
    And every ${p(Math.round(Ct/Sn))} points of <b>chirurgy</b> closes every wound on you as fast again as
    it closed at none.</p>
    ...
    trade. Every other go comes out at what the tool itself is worth &mdash; ${cr} its quality,
    and never above your skill. The hatchet you washed ashore with is quality ${s(`hatchet`)}, so
    ${w(s(`hatchet`)/W)} of your logs come out at your skill. Nothing you make is ever finer than
    ...
    roll against and comes out at ${cr} your skill, plus 1.</p>
    <h3>Rare things</h3>
    <p>Now and again a thing comes off the bench better than the hands that made it had any right to
    produce. About ${N(J.map((e,t)=>`<b>one thing in ${p(Math.round(1/ye(e)))}</b> ${t===0?`is `:``}<b>${_[e].name}</b>`))}.
    Nothing brings it on &mdash; not skill, not tools, not the metal &mdash; and nothing you do can make
    it more likely; you make ${p(Math.round(1/ye(_.length-1)))} ordinary things and find that you have one.</p>
    <p>A rare thing is <b>better at whatever it was for</b> by ${N(J.map(e=>w(_[e].boost-1))).replace(/ and ([^,]*)$/,` or $1`)}
    &mdash; an edge that bites, armour that turns aside more, a tool that works truer &mdash; <b>wears and rots
    at ${N(J.map(e=>E(_[e].keep)))}</b> of an ordinary one's pace, and can be <b>improved past the
    ceiling of your own skill</b> by ${N(J.map(e=>String(_[e].ceiling))).replace(/ and ([^,]*)$/,` or $1`)}.</p>
    ...
    ${E(oe)} a step &mdash; ${J.map(e=>`<b>${E(oe*e)}</b>`).join(`, `)}
    for rare, supreme and fantastic, and never less than <b>one more unit a step</b>, so a kiln's
    ${p(16)} becomes ${N(J.map(e=>p(Ye(16,{rare:e}))))}. A rare thing keeps
    ...
    ${O(1/Mt)} it burns, and you can <b>Take ashes</b> from it whether it is lit or cold. They pile up
    ...
    it; on dry land it will not fill. ${m(p(R(`make_lye`,`ash`)))} lots of ashes leached into a bucket of water make a
    ...
    reeds; cut it again too soon and there is nothing left to take. ${m(S(`reed`,R(`make_papyrus`,`reed`)))} soaked in a bucket of
    water, split, laid crosswise and pressed give ${p(z(`make_papyrus`))} sheets of <b>papyrus</b>, which is the
    <b>Papyrusmaking</b> skill.</p>
    <p><b>Ink</b> is the alchemist's part: a gland &mdash; the rare thing off a carcass &mdash; ground with
    ${p(R(`make_ink`,`ash`))} lots of ashes into a bucket of water, for ${p(z(`make_ink`))} lots of ink.
    ${m(p(R(`make_book`,`papyrus`)))} sheets, ${p(R(`make_book`,`leather`))} leather boards, a lot of ink and a needle bind into a <b>book</b>.</p>
    <p>Right-click a book and <b>Study</b> it. Each go takes ${O(ke(lr))}, raises <b>mind logic</b>, which is
    what decides how many jobs you can keep in your head at once, and wears the pages a little. Held in
    one hand it is hard going; at a <b>lectern</b> you get ${y(2)} as much out of the same go.</p>
    ...
    <b>title</b> at ${N(In.map(String))} &mdash; ${Nn(`carpentry`).map(e=>e.name).join(`, `)}
    ...
    <p>A <b>knack</b> comes of the work itself: <b>one go in ${p(kn)}</b>, at any trade and at any
    level, leaves one behind. It lands on the trade you were working ${w(nt)} of the time, and
    otherwise on one of the trades beside it &mdash; a long day of carpentry may leave you better at
    bowyery, because it is the same hands and the same wood. A knack is worth ${w(jt)} more on
    everything that trade teaches you from then on, it never wears off, and a trade holds
    <b>${p(5)}</b> of them: ${y(1+5*jt)} on every gain, for good. They stack with a night's rest
    ...
    <p>A cooked dish leaves a knack too, and a stronger one &mdash; ${y(1+lt)} rather than
    ${w(jt)} more &mdash; but it wears off, in ${O(di)} to ${O(fi)}. One is earned and kept;
    ...
    hold it &mdash; without the lye it washes straight out. ${C(Le.length)} of them, easiest first:
    ${N(ur.map(e=>`<b>${e.name.toLowerCase()}</b> (${e.word}, from ${S(e.from,e.count)}, alchemy ${e.difficulty})`))}.
    One boil gives ${p(z(`make_${Le[0].id}`))} pots and hands the bucket back.</p>
    ...
    ${p(R(`make_rope`,`wemp`))} fibres to a rope, on the <b>ropemaking</b> skill. ${m(S(`rope`,R(`make_thick_rope`,`rope`)))} laid up again make a <b>thick
    rope</b>, which is the hawser everything heavy hangs on.</p>
    <p>Rope is not decoration. A <b>bridle</b> takes ${p(R(`make_bridle`,`rope`))} for the reins, a <b>rowing boat</b>
    ${p(R(`make_rowing_boat`,`rope`))}, a <b>sailing boat</b> ${p(R(`make_sailing_boat`,`rope`))} and ${p(R(`make_sailing_boat`,`thick_rope`))} hawsers for her standing
    rigging, and a <b>well</b> ${S(`thick_rope`,R(`make_well`,`thick_rope`))} to hang the bucket down the shaft. Keep a field of
    ...
    <b>yarn</b>, and at the loom to weave ${p(R(`weave_cloth`,`yarn`))} yarn into a length of <b>cloth</b>. Cloth stuffs a
    mattress, sews into clothing, and twisted into a <b>bowstring</b> it is the start of every bow.</p>
    <h3>Armour</h3>
    <p>Armour is worn a piece at a time in ${p(dr.length)} places &mdash; ${N(dr)} &mdash;
    ...
    <b>cloth</b>, sewn by tailoring, ${E(P.cloth.soak)}; <b>leather</b>, cut from tanned hide with a knife
    by leatherworking, ${E(P.leather.soak)}; <b>chain</b>, poured from moulds and riveted up at an anvil by
    chain armoursmithing, ${E(P.chain.soak)}; and <b>plate</b>, beaten out whole by plate armoursmithing,
    ${E(P.plate.soak)}. Quality and the skill behind it raise all of those, and damage lowers them: armour
    wears where it is struck, and a piece beaten to nothing falls off you. Weight is the price &mdash; a full
    suit of iron plate slows you by ${E(tr/(1+tr))} and makes every action cost ${E(tr)} more
    wind, where cloth costs ${E(P.cloth.burden)}.</p>
    ...
    <p>Every weapon belongs to a kind, and each kind is its own subskill: ${N(pr.map(e=>`<b>${e}</b>`))}.
    ...
    ${p(en.find(e=>e.id===`arrow_head_mould`)?.per??0)} <b>arrow heads</b>. A club is simply carved from a log, which is what most people start with.</p>
    <p>Bows are tillered with <b>bowyery</b> from shafts and a bowstring, in ${p(mr.length)} sizes:
    ${N(mr.map((e,t)=>`a <b>${d(e.id).name.toLowerCase()}</b> ${t===0?`reaches `:``}${p(e.range??0)}${t===0?` tiles`:``}`))},
    each slower to draw and heavier in the hit than the last. Arrows are made with <b>fletching</b> from
    ${L(`make_arrows`)}, ${p(z(`make_arrows`))} to a go &mdash; and feathers come only off a bird:
    ${N(hr.map(e=>`the ${e.name}`))}. With a bow in hand, <b>Shoot</b> appears on any wild creature
    ...
    walls, smelters and kilns are built from &mdash; or, ${p(R(`make_stone_slab`,`rock_shards`))} shards at a time, a <b>slab</b>. Slabs are
    not for building: they are paving. Choose <b>Pave (slabs)</b> on any tile with a trowel in hand and
    the slab goes down as a floor of that stone, and each of the ${p(Gn.length)} looks quite different from the
    ...
    <b>Repair</b> takes out. There are ${p(g.length)} things under the island, from ${de(g[0].name)} <b>${g[0].name}</b> in
    ${p(g[0].parts)} pieces to ${de(Y.name)} <b>${Y.name}</b> in ${p(Y.parts)}, and a relic is only recognised once your archaeology has
    ...
    its mining reaches ${gr(`iron`)}. It works <b>${p(10)} units of water</b> deep, the same as you do, and a face it
    ...
    <b>shear</b> it with a knife for <b>wool</b>, which grows back in ${O(1/(A.woola.fleece??1))}. The
    <b>Ulva</b> is the first thing on this island that will come at you unprovoked: it hunts by scent
    from ${p(A.ulva.notice??7)} tiles off and does not stop until you are well away or it is badly hurt. It takes taming
    ...
    <b>mount</b> it. A green one carries you at ${y(_r/Yt)} your own pace and over the same ground;
    one whose climbing is worked right up is ${y(vr/_r)} as fast as a green one and goes up slopes you would
    ...
    both its bite and its beat: it hits ${y(Lt(q))} as hard at mastery, and its circuit grows from
    ${p(A.rowl.workRange)} tiles to ${p(A.rowl.workRange+$t(q)*(A.rowl.rangePerStep??10))}. The <b>Noot</b> is a plump upright waddler that
    ...
    readier for the next offering: <b>${E(dn)} on the chance</b> for every attempt in a row, with no
    ...
    ${O(be)}, and <b>raising a hand to it ends the run outright</b> &mdash; nothing that has been hit
    ...
    ${E(x.young.scale)} the size, moves at ${E(x.young.speed)} of the pace, grows no fleece and gives no milk, and is no use in the
    traces or under a saddle &mdash; but it has not learned to mistrust you, so it is ${y(x.young.tame)} as easy
    to tame. It is <b>grown</b> after ${O(ht)}, and everything in the book describes it then. After
    ${O(rn)} it is <b>old</b>: ${E(x.old.speed)} of the pace, ${E(x.old.pull)} of the pull in the traces and
    ${E(x.old.growth)} of the pace growing a fleece back, but heavier, and an old carcass is worth ${E(x.old.yield-1)} more
    ...
    than all of them: each adds ${E(A.shaggan.pull??.25)} to a team's pull where most beasts add ${E(vn)}.</p>
    ...
    empty bucket, and a bucket of milk presses into ${p(z(`make_cheese`))} <b>cheeses</b>. The <b>Vesp</b> is a swarm
    ...
    <b>beeswax</b> &mdash; and ${L(`make_candle`)} draw ${p(z(`make_candle`))} <b>candles</b>. The
    <b>Lume</b> is only ever out after dark and carries its own light about with it: keep one and it
    lights ${p(A.lume.glow??0)} tiles round itself however dark it is.</p>
    ...
    border out ${p(2)} tiles and lets ${l(2)-l(1)===1?`one`:p(l(2)-l(1))} more wildermon work the deed, and each is earned by building the
    ...
    <p>A deed worker feeds itself: once its belly falls below ${w($e)} it goes to whichever crate on
    ...
    <p>A deed worker starts within ${p(br[0])} to ${p(br[br.length-1])} tiles of the token, by its sort, and earns another
    10 tiles of range for every 10 levels of its task skill &mdash; ${xr} &mdash; so a
    ...
    with a mallet from ${r}; it weighs <b>${d(Ze).weight} kg</b> and does not rot.
    ...
    where you want it; it snaps to the grid. They hold ${D.log.capacity} and ${D.plank.capacity} things, can be opened, emptied and
    ...
    <b>one more pass over the tile for every ${p(20)} points</b> of the skill: ${Sr}.
    ...
    sit, and choose <b>Sit and think about nothing</b>. You may sit once every ${O(It)}, and
    <b>where</b> you sit decides what it is worth: your own yard ${E(k.yard)} of a sitting anywhere else,
    high ground off your settlement ${y(k.high)} as much, and where the ground runs out and the air is thin
    <b>${y(k.thin)}</b> as much; and your feet in the water add ${E(k.water-1)} to wherever that is.</p>
    ...
    <table>
      ${M.map(e=>`<tr><td><b>${e.name}</b></td><td>${e.note} ${e.steps.map(e=>e.ability?`<b>${e.name}</b> (${e.at}, then ${O(e.ability.rest)} before it again): ${Cr(e.note)}`:`<i>${e.name}</i> (${e.at}): ${Cr(e.note)}`).join(` `)}</td></tr>`).join(`
      `)}
    </table>
    ...
    <b>favour</b>, on the <b>prayer</b> skill. You may say what you have to say once every ${O(cn)}, and it is
    worth most at <b>${N(Et.map(Xn))}</b> &mdash; ${y((Tn+_t)/Tn)} what it is worth
    ...
    &mdash; ${Math.round(Zt(K(Jt)))} at the start and 120 at the very top.</p>
    <p>${C(ft.length)} things it buys, and none of them can be had any other way:</p>
    <table>
      ${ft.map(e=>`<tr><td><b>${e.name}</b></td><td>${e.cost} favour, prayer ${e.level}. ${e.note}</td></tr>`).join(`
      `)}
    </table>
    <h3>The things that are not wildermon</h3>
    <p>Most of what walks this island can be tamed. ${C(wr.length)} things cannot. A <b>goblin</b> is knee-high and
    ...
    They notice you from ${p(Tr[0])} to ${p(Tr[Tr.length-1])} tiles off, where anything else that hunts has your scent at
    ${p(7)}; they come straight at you, and they do not give up easily.</p>
    <p>They are <b>rare</b>: ${E(Cn)} of what stands up out in the country, and of those
    ${N(v.map(([e,t])=>`${E(t/Er)} ${A[e].name.toLowerCase()}s`))}. Only so many of each are alive at once
    (${N(v.map(([t])=>`${p(e[t]??1)} ${A[t].name.toLowerCase()}${(e[t]??1)===1?``:`s`}`))}), and the bigger the thing the further it keeps
    from your token &mdash; ${N(v.map(([e])=>`${de(A[e].name)} ${A[e].name.toLowerCase()} ${p(wt[e]??0)} tiles`))}. None of them can be tamed,
    trapped, bred or brushed. There is nothing to be done with one but kill it, and nothing to be gained
    by meeting one in your shirt: ${N(v.map(([e],t)=>`${de(A[e].name)} ${A[e].name.toLowerCase()}'s ${t===0?`blow takes `:``}${E(Math.round(A[e].attack*qe*100)/100)}`))} of an unarmoured life.</p>
    <p>What they are worth is on the other side of that. Butchering one gives what a wildermon gives and
    then some: <b>tusk</b> and <b>sinew</b> off an orc or an ogre, and off a dragon ${p(A.dragon.butcher.scale??0)}
    <b>dragon scales</b> and a <b>hoard</b> &mdash; ${p(4)} to ${p(10)} lumps, by how much of the carcass you keep,
    of the ${p(Dr.length)} deep metals and the ${p(we.length-Dr.length)} precious ones, which is the only place on the
    island they turn up together.</p>
    <p>Tusk and sinew make a <b>composite bow</b>: ${L(`make_composite_bow`)}, at ${Jn(`make_composite_bow`)}. It
    throws an arrow <b>${p(Or.range??0)}</b> tiles for ${Or.damage} damage, where the best wooden bow throws
    ${p(kr.range??0)} for ${kr.damage}. Dragon scale riveted to leather makes <b>scale armour</b>, a class above
    plate: it turns <b>${E(P.scale.soak)}</b> of a blow where plate turns ${E(P.plate.soak)}, and it burdens you
    less than chain. A full suit takes ${p(Ar)} scales, where one dragon carries ${p(A.dragon.butcher.scale??0)}.</p>
    <h3>Traps</h3>
    ...
    <p>A <b>snare</b> is a noose of rope on a bent shaft: ${pe(f.snare.bill)}, and it holds anything
    up to about <b>taming ${f.snare.holds}</b>. A <b>deadfall</b> is a weighted board on a trigger &mdash;
    ${pe(f.deadfall.bill)}, with a mallet &mdash; and it holds to about <b>taming ${f.deadfall.holds}</b>.
    ...
    <p>A trap rots where it stands, ${O(f.snare.lifeMin)} to ${O(f.snare.lifeMax)} for a snare and up to
    ${O(f.deadfall.lifeMax)} for a good deadfall, and whatever is in it walks away when it goes over. A <b>timid</b>
    creature &mdash; the very thing you cannot walk up to &mdash; is ${y(se)} as likely to walk into
    one, and a hunter ${E(je)} as likely. Getting the catch out is still <b>taming</b>: the skill wall stands
    ...
    traits are the whole difference between one Roxxen and the next. A trait sits in one of ${p(h.length)} tiers
    &mdash; ${N(h.map(e=>`<b>${e}</b>`))} &mdash; and what it is worth
    climbs steeply with the tier: a common trait moves its number by ${E(jr.common[0])} to ${E(jr.common[1])},
    a fantastic one by ${E(jr.fantastic[0])} to ${E(jr.fantastic[1])}. Traits lift how fast it <b>moves</b>, how quickly it <b>works</b>, how fast what it does
    ...
    <p>${C(Ve.length)} of the traits are <b>fighting blood</b> &mdash; fanged, plated, slippery, quick-jawed,
    ...
    (rare), fanged (supreme) or fanged (fantastic), each worth steeply more than the last &mdash; ${N(h.map(e=>y(te[e])).slice(1))} what
    the common grade is &mdash; and a line is bred up a grade at a time under the same husbandry.
    ${m(E(ie))} of what the wild throws up is fighting blood, and no animal carries more than one grade of a name.</p>
    ...
    works quicker, learns faster and heals as you go over it. Care runs out again over ${O(3*Yn)}
    ...
    both <b>grown</b>, both <b>fed</b>, and neither put to a mate in the last ${O(le)}, then
    ...
    the female carries for ${O(Je)} and then drops a young one, and what it is born with was
    settled at that moment &mdash; a sire sold, released or eaten in between has already had his say.</p>
    <p>${m(p(3))} slots are filled one at a time. Each is drawn from what the pair carry between them, and
    husbandry decides <b>how often the parents' blood comes through</b> rather than whatever the wild throws up
    (${w(Vt(0,0))} at no skill, ${E(Vt(q,1))} at ${p(q)} with a well-brushed pair), and <b>how often a
    trait comes through one tier better than either parent had it</b> (never at no skill and with no care,
    ${E(Se(q,1))} at ${p(q)} with a well-brushed pair) &mdash; commons becoming rares, rares becoming supremes.
    ...
    enough to recognise: common blood is plain to anybody, ${N(h.slice(1).map((e,t)=>`${e} ${t===0?`takes `:``}${ue[e]+1}`))}
    ...
    back carries ${p(Math.floor(220/d(`dirt`).weight))} before it starts to drag, and moving a bank is a great many walks. Dropping dirt and the packing-in half
    ...
    <p>${C(X.length)} kinds of seam lie in the rock, and each needs a certain <b>mining</b> skill before it can be
    worked at all: ${N(X.filter(e=>e.level===X[0].level).map(e=>Mr(e.name)))} from the very start, then
    ${N(X.filter(e=>e.level!==X[0].level).map(e=>`${Mr(e.name)} at ${e.level}`))}. <b>Iron is
    ${E(Pt(`iron_ore`))} of every seam on the island</b> and everything else shares what
    ...
    ${Math.round(1/Ce)}</b> a slab comes away of its own accord and the corner drops a
    ...
    than working it, and it gives way about one attempt in ${p(Math.round(1/Ht))}. What breaks away is yours either
    way.</p>
    ...
    ${p(Math.round(1/Tt.land))} carries some seam, and one sea tile in ${p(Math.round(1/Tt.water))}. <b>Prospecting</b> is how you read it. It marks every
    ...
    ${N(Nr.map((e,t)=>`<b>${Fr(e)}</b> at ${t===0?`level `:``}${e}`))} &mdash;
    ${p(Math.round(1/Pr(Nr[0])))} goes for the first point of a skill, ${p(Math.round(1/Pr(Nr[2])))} for the point after
    ${Nr[2]}, and something like <b>${p(Math.round(1/Xt))}</b> for the last. Nobody finishes a skill in passing; the last
