import{$ as e,$a as t,$i as n,$n as r,$s as i,$t as a,Aa as o,Ai as s,Ar as ee,At as c,Bo as l,Br as te,Ca as ne,Cc as re,Ci as ie,Cn as ae,Ct as u,Di as oe,Do as se,Dt as ce,Eo as le,Et as ue,Fi as de,Ft as fe,G as pe,Gn as me,Hi as he,Hr as d,Hs as ge,Ht as _e,Ia as f,Ii as ve,Ji as ye,Jo as p,Js as be,K as xe,Li as Se,Ln as Ce,Lo as we,Mi as Te,Ni as Ee,No as De,Nt as Oe,Oo as ke,Os as Ae,Ot as je,P as Me,Pi as m,Pn as Ne,Po as h,Pt as Pe,Q as g,Qa as Fe,Qs as _,Qt as Ie,Ri as Le,Rn as Re,Ro as ze,Si as Be,So as Ve,St as He,Ts as Ue,Tt as We,U as v,Vi as y,Vn as Ge,Vo as Ke,Vr as qe,Vt as Je,W as Ye,Wr as Xe,Ws as Ze,X as Qe,Xs as b,Xt as $e,Ya as et,Yo as x,Yt as tt,Z as nt,Zn as rt,Zo as S,Zs as it,_i as at,_r as ot,_s as st,_t as ct,ac as lt,ar as ut,as as dt,bc as ft,bi as pt,bn as mt,bo as C,bt as ht,cc as gt,ci as _t,cs as vt,ct as yt,d as bt,da as xt,dc as St,ds as Ct,ea as wt,ec as w,en as Tt,es as Et,et as Dt,fa as Ot,fo as kt,fs as At,ga as jt,gi as Mt,gn as Nt,gs as Pt,gt as Ft,ha as It,hi as Lt,hn as Rt,ho as T,ht as zt,ic as E,ir as Bt,is as Vt,it as D,ji as Ht,jo as Ut,jr as O,js as Wt,jt as Gt,ko as Kt,kt as qt,la as Jt,li as Yt,ln as Xt,ls as Zt,ma as Qt,mi as $t,mo as en,ms as tn,mt as nn,nc as k,ns as rn,nt as an,o as on,oc as sn,or as cn,os as ln,p as un,pa as dn,po as A,q as fn,rc as j,ro as M,rt as pn,ss as mn,t as hn,tc as N,ti as gn,tn as _n,to as P,tt as vn,u as yn,ua as bn,ui as xn,us as Sn,va as Cn,vi as wn,vn as Tn,vr as En,vs as Dn,vt as On,wi as kn,wr as An,wt as jn,x as Mn,xa as Nn,xc as Pn,xo as Fn,xt as In,yi as Ln,yn as Rn,ys as zn,yt as Bn,zo as Vn,zr as Hn}from"./boards.js";import{B as Un,F as Wn,G as F,H as Gn,R as Kn,Y as qn}from"./supabase.js";var Jn=()=>`${+(450/60).toFixed(1)} minutes`,Yn=e=>{let t=O.get(e);if(!t)throw Error(`The help names a recipe there is not: ${e}`);return t},I=(e,t=!1)=>p(Yn(e).inputs.map(e=>[e.item,e.count??1]),t),L=(e,t)=>{let n=Yn(e).inputs.find(e=>e.item===t);if(!n)throw Error(`The help says ${e} takes ${t}, and it does not.`);return n.count??1},R=e=>Yn(e).count??1,Xn=e=>{let t=Yn(e);return`${(Pn.get(t.skill)?.name??t.skill).toLowerCase()} ${t.difficulty??0}`},z=e=>{let t=be.get(e);if(!t)throw Error(`The help names a material there is not: ${e}`);return t},B=e=>Un[e],V=e=>1-qn(B(e).roll,1),H=(e,t)=>S(e).feeds?.[t]??0,U=100,W=1,Zn=3600,G=e=>`${String(Math.floor(e)).padStart(2,`0`)}:${String(Math.round(e%1*60)).padStart(2,`0`)}`,Qn=Object.values(D).filter(e=>e.glow),$n=Object.values(D).filter(e=>e.wound===`burn`),er=18/24,K=e=>Pn.get(e)?.start??1,q=100,tr=100,nr=A.plate.burden*z(`iron`).weight,rr=e=>Math.round(100/(Oe(e)*z(`copper`).wear)/10)*10,ir=80,ar=90,or=e=>Math.ceil(ir/Ge(e).healed),sr=e=>`${+(ir*Ge(e).cost).toFixed(1)} points`,cr=Object.values(At),lr=`${u} to ${+(u+jn).toFixed(2)} times`,J=l.map((e,t)=>t).slice(1),ur=De.get(`study_book`)?.baseTime??0,dr=[...Ze].sort((e,t)=>e.difficulty-t.difficulty),fr=[...new Set(kt.map(e=>e.slot))],pr=Object.values(A).filter(e=>!kt.some(t=>t.cls===e.id&&O.get(`make_${t.id}`)?.inputs.some(e=>e.item===`dragon_scale`))),mr=[...new Set(T.map(e=>e.kind))],hr=T.filter(e=>e.ammo&&O.get(`make_${e.id}`)?.inputs.some(e=>e.item===`bow_string`)),gr=Object.values(D).filter(e=>e.butcher?.feather||e.shearYield===`feather`),Y=h[h.length-1],_r=e=>sn.find(t=>t.id===e)?.level??0,vr=D.orse.speed*ce(0),yr=Math.min(5,D.orse.speed*ce(q)),br=i(Object.entries(r).map(([e,t],n)=>`level ${e} ${n===0?`wants `:``}${i(t.map(e=>`${e.label[0].toLowerCase()}${e.label.slice(1)}`))}`)),xr=[...new Set(Object.values(D).filter(e=>!e.monster).map(e=>e.workRange))].sort((e,t)=>e-t),Sr=(()=>{let e=new Map;for(let t of Object.values(D))t.rangePerStep&&t.rangePerStep!==10&&e.set(t.rangePerStep,[...e.get(t.rangePerStep)??[],`a ${t.name}`]);return i([...e.entries()].sort((e,t)=>e[0]-t[0]).map(([e,t])=>`${i(t).replace(/ and ([^,]*)$/,` or $1`)} ${e}`))})(),Cr=i([0,20,40,q].map((e,t)=>t===0?`${w(ne(e))} pass below ${w(20)}`:`${w(ne(e))} at ${w(e)}`)),wr=e=>e&&`${e[0].toLowerCase()}${e.slice(1)}`,Tr=Object.values(D).filter(e=>e.monster),Er=Tr.map(e=>e.notice??7).sort((e,t)=>e-t),Dr=g.reduce((e,[,t])=>e+t,0),Or=ze.filter(e=>!lt.some(t=>e===`${t}_lump`)),kr=T.find(e=>e.id===`composite_bow`)??T[0],Ar=[...hr].sort((e,t)=>(t.range??0)-(e.range??0))[0]??T[0],jr=kt.filter(e=>e.cls===`scale`).reduce((e,t)=>e+L(`make_${t.id}`,`dragon_scale`),0),Mr=Object.fromEntries(m.map(e=>{let t=de.filter(t=>t.tier===e).map(e=>Math.max(...Object.values(e.effects).map(e=>Math.abs((e??1)-1))));return[e,[Math.min(...t),Math.max(...t)]]})),X=Wn.filter(e=>e.level).sort((e,t)=>(e.level??0)-(t.level??0)),Nr=e=>e.replace(/ (vein|seam)$/,``).toLowerCase(),Pr=[1,50,90,99],Fr=e=>re(e,He,1),Ir=e=>String(Number(Fr(e).toPrecision(Fr(e)>=.1?2:1))),Lr=()=>{let e=f(gn).bill.map(([e,t])=>`<b>${t} × ${S(e).name.toLowerCase()}</b>`);return`${e.slice(0,-1).join(`, `)} and ${e[e.length-1]}`},Rr=xt[bn[0].id].length,zr=dn/Rr,Br=e=>Ot.find(t=>t.rank===e)?.cost??0,Vr=e=>Br(e)===1?`a point`:`${w(Br(e))} points`,Hr=(e,t)=>Ot.find(n=>n.channel===e&&n.rank===t)?.note??``,Ur=[...new Set(Qt.map(e=>Object.keys(e.muls).length))].sort((e,t)=>e-t),Wr=e=>Object.entries(jt(e)?.muls??{}).map(([e,t])=>`${Jt[e].note.toLowerCase()} ${It(t)}`).join(`, `),Gr=Pn.get(`awareness`)?.start??20,Kr=Object.keys(Vn).filter(e=>Vn[e].holds),qr=o.filter(e=>O.get(`make_${e.id}`)?.skill===`fine_carpentry`),Jr=qr.filter(e=>e.capacity&&!e.liquid).sort((e,t)=>(e.capacity??0)-(t.capacity??0)),Yr=Object.keys(Ae).length,Xr=o.filter(e=>e.boat),Zr=f(`rowing_boat`),Z=f(`sailing_boat`),Q=f(`caravel`),Qr=f(`large_cart`),$=f(`wagon`),$r=o.filter(e=>e.liquid).sort((e,t)=>(e.liquid??0)-(t.liquid??0)),ei=f(`craft_bin`).heft??0,ti=f(`seed_bin`).heft??0,ni=f(`sprout_bin`).heft??0,ri=11,ii=(Mn.Gold??0)/(Mn.Silver??1),ai=5,oi=[...St].filter(e=>!lt.includes(e)),si=40,ci=70,li=wt.get(`cider`)??n[0],ui=Gn.filter(e=>e.fruit),di=se.flatMap(e=>[Ut(e,1),Ut(e,U)]),fi=Math.min(...di),pi=Math.max(...di),mi=e=>{let t=gt.find(t=>t.id===e);if(!t)throw Error(`The help names a mould there is not: ${e}`);return`poured at the smelter and beaten out on an anvil, ${t.lumps===1?`a lump of metal`:`${w(t.lumps)} lumps of metal`} ${t.per?`to ${w(t.per)}`:`to each`}`},hi=e=>{let t=e=>e.speed*qn(e.roll,1);return 60*t(B(F.Slabs))/t(B(e))},gi={ql:50,force:.6},_i=i([Math.PI/2,Math.PI,Pe,0].map(e=>(Z.boat?qt(Z.boat,gi.ql,20,_e(gi.force)*fe(e),0):0).toFixed(1))),vi=()=>{let e=Object.values(ve).map(e=>`<b>${e.label}</b> (${e.means})`);return`${e.slice(0,-1).join(`, `)} or ${e[e.length-1]}`};function yi(){let r=Lr(),ne=vi();return`
    ...
    <table>
      ...
      <tr><td><kbd>Q</kbd> <kbd>E</kbd></td><td>Turn the view ${k(1/8)} of a turn; the compass shows north</td></tr>
    </table>
    ...
    ${w(64)} of them; past that the oldest is pushed off.</p>
    ...
    under ${N(bt)} of the health it can carry, a <b>hungry</b> one on anything under ${N(yn)} fed, and the card
    ...
    <p>You may found <b>one</b> settlement of your own and be a citizen of <b>${w(3)}</b> others, whether
    ...
    ${Jn()}, the island counts you as gone, and from then until you come back it keeps count for
    ...
    every 30 seconds while it is open. <b>Skills:</b> ${hn.skills}
    <b>Wildermon:</b> ${hn.wildermon} <b>Settlements:</b> ${hn.settlements}
    ...
    &mdash; <kbd>1</kbd> to <kbd>9</kbd> and <kbd>0</kbd> do the <b>first ${w(un.length)} things that can actually
    ...
    the handful as well as one at a time: <b>once</b>, ${on.map(w).join(`, `)}, or <b>until you
    ...
    jobs you do most. It has <b>one loop for every ${w(10)} points</b> of how well it was made, to a full ${w(10)}
    ...
    bar above the action shows what is waiting. You can keep <b>${w(3)}</b> jobs in your head to begin
    with, and one more for every ${w(10)} points of <b>mind logic</b>, which is earned by crafting. Walking
    ...
    read a dark hillside, so it <b>starts at ${Gr}</b> where the others start at 20 &mdash; and the
    ${w(15)} tiles on the flat that this island used to give everybody is what it gives somebody at the
    top of it. At ${Gr} you see ${N(_n(Gr))} of that &mdash; ${w(Math.round(15*_n(Gr)))}
    ...
    is ${w(Math.ceil(1/Tt))} trees deep to the eye, and the <b>dark takes ${k(a)} of everything</b>. Your
    ...
    <p>Each trade earns <b>points</b> off the best skill it covers, and its tree is <b>${w(Rr)} columns of
    ${w(zr)}</b>. A column is one <b>channel</b>: one number in the rules, and every card names it and says
    exactly what that node does to it &mdash; ${[`hands`,`aim`,`knit`].map((e,t)=>`<i>${Hr(e,[1,1,zr][t])}</i>`).join(`, `)}.
    The lower nodes cost ${Vr(1)} each, the one above them costs ${w(Br(zr))}, and the lower must be
    bought first. A few channels are better lower &mdash; ${i(Object.values(Jt).filter(e=>e.lower).map(e=>e.note.toLowerCase()))}
    &mdash; and the sign on the card is the change to the number, so those read as a minus.</p>
    <p>A <b>rite</b> is the one thing a trade may ask for out loud: ${i(Ur.map(w)).replace(` and `,` or `)} channels pushed much
    ...
    for another &mdash; ${jt(`redhour`)?.name} is <i>${Wr(`redhour`)}</i> &mdash; so read both halves before you call one.
    Only the <b>fighting</b> trades have a rite; a craft trade has its ${w(dn)} nodes and none. It sits at
    ...
    and the well, and <i>nail</i> finds all ${w(ee.filter(e=>e.inputs.some(e=>e.item===`nail`)).length)} recipes that want nails. The count at the foot
    ...
    (<kbd>J</kbd>) is that list: ${w(_t.length)} goals in ${w(Yt.length)} chapters, from felling your first tree to
    ...
    <p><b>The ${w(Gn.filter(e=>!e.fruit).length)} woods that bear nothing.</b> <b>Pine</b> is soft, light and quick to work, and rots as fast as it grew.
    <b>Willow</b> and <b>birch</b> are light and springy. <b>Maple</b> is even-tempered. <b>Oak</b> is
    hard going and worth it: an oak thing takes ${N(z(`oak`).wear/z(`pine`).wear)} of the knocks a pine one does, holds more
    ...
    <p><b>The ${w(sn.length)} metals.</b> <b>Copper</b> is what everything starts in and is soft with it.
    ...
    which is iron with coal beaten through it, keeps an edge ${E(z(`copper`).wear/z(`steel`).wear)} as long as copper and does not
    mind the weather. <b>Bronze</b> and <b>brass</b> are the first alloys worth a crucible. <b>Silver</b> hardly
    tarnishes and bites anything that carries its own light ${E(en)} as hard, which is what a
    <b>Lume</b> or an <b>Embra</b> is. <b>Gold</b> rots at ${N(z(`gold`).decay/z(`copper`).decay)} of copper's pace, weighs
    ${E(z(`gold`).weight/z(`copper`).weight)} what copper does and is good for nothing else. The ${w(oi.length)} out of the deep seams are what a
    lifetime of mining is for: <b>adamantine</b> takes the keenest edge, <b>glimmersteel</b> is light and turns
    aside ${N(z(`glimmersteel`).soak/z(`copper`).soak-1)} more than copper does, <b>mithril</b> is lighter than the wood it is hafted to, and
    <b>seryll</b> scarcely takes a mark at all. A tool's metal decides how fast and how true it works, so a
    bronze hatchet at ${si} works as well as a copper one at ${Math.round(si*z(`bronze`).bite/z(`copper`).bite)}; a weapon's metal decides
    what it does; armour's metal decides both what it stops and what it costs you to carry.</p>
    <h3>Brewing</h3>
    <p>Fill a barrel from a well or the shore, stand at it and <b>set a brew going</b>. ${b(n.length)} of them:
    ${i(n.map(e=>`<b>${e.name.toLowerCase()}</b> from ${x(e.input,e.count,!0)} and ${e.litres} litres of water in ${j(e.time)}`))}.
    Each gives back as many litres of drink as it took of water.</p>
    <p>A <b>quern</b> presses fruit too: ${w(L(`press_apple_juice`,`apple`))} of any fruit into a <b>bucket of juice</b>, sweet and with
    nothing dangerous in it, and ${w(L(`press_apple_cider`,`apple`))} apples or pears straight into a <b>bucket of cider</b> with no
    barrel and no waiting &mdash; one bucket, where the barrel would have made ${w(li.litres/5)} from ${x(li.input,li.count)}
    and ${w(li.litres)} litres of water.</p>
    ...
    and a brew carries it far longer than food &mdash; at quality ${ci}, a baked potato's lasts
    ${j(Ut(`baked_potato`,ci))} and a bucket of wine's ${j(Ut(`wine_bucket`,ci))}. A barrel of the right thing
    ...
    <p>${b(y.length)} fish run at ${w(new Set(y.map(e=>e.depth)).size)} depths, and each wants a hand to match:
    ${i(y.map(e=>`<b>${e.name.toLowerCase()}</b> ${e.depth>0?`from ${w(e.depth)} deep`:`anywhere there is water`}${e.level>y[0].level?` with fishing ${e.level}`:``}`))}.
    ...
    what comes off it follows the size of the fish &mdash; a pike is ${w(R(`cook_pike`))} helpings and a sturgeon
    ${w(R(`cook_sturgeon`))}.</p>
    ...
    all of it, a sturgeon is ${N(he(`sturgeon`))} of what takes a bare hook and ${N(he(`sturgeon`,`perch`))} of what
    takes a perch.</p>
    ...
    are elsewhere, holds ${w(d.creel.hold??0)}, and gives about <b>${w(Math.round(1/Hn))} fish to a baiting</b> before the bait is
    worked out of it. Empty it from the bank. Kept baited and emptied, the best one that can be woven is
    worth around ${w(Math.round(Zn/45*Xe(U)))} fish an hour for no work at all.</p>
    ...
    <h3>Fruit trees</h3>
    <p>${b(ui.length)} of the ${w(Gn.length)} trees bear. <b>Apple</b> and <b>olive</b> grow wild here and there in
    ...
    it to the crate.</p>
    <p>A sapling bears nothing; leave it to grow. A mature tree gives about ${w(3)} of its fruit to a
    picking and an old one about ${w(5)}, more as your forestry rises, and a picked tree needs a few
    minutes before there is anything on it again. ${_(x(`apple`,L(`make_apple_pie`,`apple`)))} and a dough bake into ${w(R(`make_apple_pie`))} <b>apple
    pies</b>, the best food on the island; ${x(`cherry`,L(`make_cherry_preserves`,`cherry`))} boil down into ${w(R(`make_cherry_preserves`))} jars of <b>preserves</b>;
    and ${x(`olive`,L(`press_olives`,`olive`))} crushed under a <b>quern</b> give ${w(R(`press_olives`))} measures of <b>olive oil</b>, which keeps almost
    for ever.</p>
    ...
    other &mdash; and a field's menu offers to <b>Sow</b> whichever you carry. A crop goes through ${w(t.length)}
    stages: ${i(t)}, each drawn differently, and every crop takes its own time
    ...
    gives ${P(0).produce} crop and ${P(0).seeds} seed, while one tended at every stage gives <b>${P(3).produce} crops and ${P(3).seeds} seeds</b>. Tending
    ...
    <p>Right-click any dry, open spot and choose <b>Build campfire</b> to lay one from ${w(2)} shafts; it
    fills a ${w(2)} by ${w(2)} block of the tile's spots. Feed it anything that burns &mdash; thatch, shafts,
    ...
    fire is the place to <b>Cook</b>: raw meat becomes cooked meat that fills ${N(S(`cooked_meat`).food??0)} of the food bar where
    raw fills ${N(S(`meat`).food??0)}, potatoes bake
    ...
    <b>Milling</b> skill. ${_(x(`wheat`,L(`make_flour`,`wheat`)))} grind down to a lot of <b>flour</b>, ${x(`corn`,L(`make_cornmeal`,`corn`))} to <b>cornmeal</b>,
    and a badly ground batch is nothing but grit.</p>
    <p>Flour and a bucket of water are worked into <b>dough</b> &mdash; ${w(R(`make_dough`))} rounds at a time, and the
    ...
    ${w(R(`make_porridge`))} bowls of <b>porridge</b>. Both are cooking rather than milling: the mill only makes the meal.</p>
    <h3>The oven</h3>
    ...
    ${w(f(`oven`).w*f(`oven`).h)} spots. Feed it anything a fire takes, peat and coal included &mdash; it holds ${j(ye)} of
    ...
    anything else, carried, and <b>set down</b> on ${w(6)} spots of a tile on your own deed. Take it up again
    ...
    metal, which you set down on ${w(4)} spots of a tile.</p>
    ...
    and otherwise at the knife's QL times ${u.toFixed(1)} to ${(u+jn).toFixed(1)}, never above
    your Butchering; with bare hands it comes off at your Butchering times ${u.toFixed(1)} to
    ${(u+jn).toFixed(1)}, plus 1. The menu's <b>Butcher</b> line says the range for the knife you carry.
    ...
    on its land. Things left outside on deed land rot ${E(1/we)} slower.</p>
    ...
    the paving skill. Another storey can only be planned once every wall of the storey below is built, up to ${w(10)} in all.
    ...
    on packed ground or in the long grass &mdash; and choose <b>Plan fence</b>. A log fence is ${w(ge(`log`,`fence`).needed.log)} logs
    where a log wall is ${w(ge(`log`,`solid`).needed.log)}; a half wall is ${k(Wt.get(`half_wall`)?.factor??0)} of one. Both stop anything alive at that border,
    ...
    from sand at a smelter: it is a gang mould with ${w(5)} channels in it, so one lump of metal
    poured into it there and beaten out on an anvil gives ${w(5)} nails at ${w(Math.round(S(`nail`).weight*1e3))} grams apiece. Crates, tool heads fitted to their
    ...
    a mallet, planks, timbers, shafts and nails it builds ${w(qr.length)} pieces &mdash;
    ${i(qr.map(e=>e.name.toLowerCase()))}. Each is carried like a crate and
    <b>set down</b> on a block of subtiles: right-click a tile and choose <b>Set furniture down</b>, and
    the piece follows the cursor until you click it down &mdash; <b>Q</b> and <b>E</b> turn it ${k(1/Yr)}, Escape
    keeps it. A piece stands the way it was set however the view is turned, and <b>Turn it</b> on a standing
    piece turns it ${k(1/Yr)} round. Staircases and ladders are planned the same way, Q and E
    choosing the side you climb from.</p>
    <p>${b(Jr.length)} of them hold things: ${i(Jr.map((e,t)=>`${e.name.endsWith(`s`)?``:`a `}${e.name.toLowerCase()} ${t===0?`takes `:``}${e.capacity}`))},
    ...
    a road wants.</p>
    <h3>Bags</h3>
    <p>${b(Kr.length)} things hold other things and are carried in your pack:
    ${i(Kr.map(e=>`a <b>${S(e).name.toLowerCase()}</b> of ${I(`make_${e}`,!0)} (${S(e).holds} things)`))}.
    ...
    ${i(Kr.map((e,t)=>`${k(S(e).shelter??1)}${t===0?` the rate`:``} in a ${S(e).name.toLowerCase()}`))}.
    ...
    short rein &mdash; <b>${An(1)} tiles round a rough post and ${An(U)} round the best</b>, however much the creature
    ...
    all the way home.</p>
    <p>Nothing holds a post up and it <b>rots where it stands</b>: <b>${j(En)}</b> for the
    roughest and <b>${j(ot)}</b> for the best that can be made, leaning further as it goes, with one
    ...
    <p>A <b>raw material bin</b> holds <b>${f(`bulk_bin`).capacity}</b> of what comes out of the ground, off a tree, out of a
    ...
    <p>It is the first of the ${w(o.filter(e=>e.heft).length)} stores that <b>do not count what is in them</b>. It weighs it:
    <b>${ei} kg</b>, which is ${w(Math.round(ei/S(`nail`).weight))} nails or ${w(Math.round(ei/S(`plank`).weight))} planks, and there is no
    ...
    <p>A <b>seed bin</b> and a <b>sprout bin</b> are the small pair, ${w(f(`seed_bin`).w*f(`seed_bin`).h)} subtile each, ${I(`make_seed_bin`)}
    apiece. Both weigh what is in them rather than counting it: the seed bin <b>${ti} kg</b>, which is
    ${Math.round(ti/S(`wheat_seed`).weight)} wheat seeds or ${Math.round(ti/S(`potato_seed`).weight)} seed potatoes, and the sprout bin <b>${ni} kg</b>, which is
    ${Math.round(ni/S(`sprout`).weight)} sprouts. The seed bin takes the
    ${w(Fe.size)} sowable seeds and nothing else; the sprout bin takes sprouts and nothing else.</p>
    <p>A <b>trash crate</b> is built with a rotten bottom on purpose: anything put in it rots <b>${E(f(`trash_crate`).trash??1)}
    ...
    <p>A <b>small cart</b> holds ${f(`cart`).capacity} things and, once you <b>take hold of it</b>, follows you wherever
    ...
    the ends must be within ${w(12)} height units of each other, and everything between must be at
    least ${w(3)} units below the deck &mdash; a gap, not a slope. ${b(Object.keys(me).length)} kinds:
    ${i(Object.values(me).map(e=>`a <b>${e.name.toLowerCase()}</b> (${p(e.bill)} a span${e.tool===`trowel`?`, with a trowel`:``}) goes <b>${w(e.span)}</b> tiles and ${e.carts?`carries a cart`:`takes foot traffic only`}`))}.</p>
    ...
    <h3>Boats</h3>
    <p>${b(Xr.length)} hulls, all of them a carpenter's work. A <b>rowing boat</b> is ${I(`make_rowing_boat`,!0)};
    she carries <b>${Zr.capacity} things</b>, wants <b>${w(Zr.boat?.draught??0)} deep</b> of water under her and is rowed, so your
    <b>body strength</b> is the engine. A <b>sailing boat</b> is ${I(`make_sailing_boat`,!0)};
    she carries <b>${Z.capacity}</b>, wants <b>${w(Z.boat?.draught??0)} deep</b>, and the wind
    ...
    <b>${w(Q.boat?.draught??0)} deep</b>, makes <b>${Q.boat?.speed}</b> tiles a second at a fair effort to the
    sailing boat's <b>${Z.boat?.speed}</b>, and carries <b>${w(Q.boat?.passengers??0)} passengers</b> besides
    ...
    ${w(Q.boat?.passengers??0)} places on deck that is free, and from then on you go where she goes and your own
    ...
    steady and slower; hard up into it is hard work; and inside the last ${w(Math.round(Pe*180/Math.PI))} degrees she is
    ...
    and do the same on the other. A sailing boat of quality ${gi.ql} with body control 20 at the tiller, in
    ${Je(gi.force)}, makes ${_i} tiles a second on a beam reach, running, close-hauled and in irons.</p>
    ...
    <p>A sailing boat <b>holds ${w(Z.capacity??0)} things</b>, crates included, and that is what she is for
    &mdash; but a hull loaded to her marks is ${N(ct)} slower than one running empty.</p>
    <h3>Why a road is worth its stone</h3>
    <p>Feet hardly care what is under them: sand is walked at ${N(B(F.Sand).speed)} of the pace of grass and laid
    stone at ${N(B(F.Slabs).speed)}, and only a bog really tells, at ${N(B(F.Marsh).speed)}. A <b>laden wheel</b> cares about very
    ...
    <p>Stone slabs and cobble cost a full wagon <b>nothing</b>. Packed dirt costs ${k(V(F.PackedDirt))}, bare
    grass ${k(V(F.Grass))}, sand ${k(V(F.Sand))}, a tilled field ${k(V(F.Field))}, and a <b>bog ${k(V(F.Marsh))}</b>. What a full
    wagon crosses in a minute on a paved road takes it <b>${j(hi(F.Grass))} over grass and
    ${j(hi(F.Marsh))} through marsh</b> &mdash; which is the whole argument for paving, and why the stone is
    worth cutting.</p>
    ...
    <p>A <b>large cart</b> takes ${I(`make_large_cart`,!0)}. It holds <b>${Qr.capacity} things of any weight</b> and has ${w(Qr.vehicle?.yokes??0)} yokes:
    ${w(Qr.vehicle?.needs??0)} wildermon will move it, ${w(Qr.vehicle?.yokes??0)} move it faster.</p>
    <p>A <b>wagon</b> takes ${I(`make_wagon`,!0)}. It holds <b>${$.capacity} things</b> and will not stir until
    <b>${$.vehicle?et($.vehicle):``} yokes</b> have a wildermon in them.</p>
    ...
    drags its feet, so feed the team before it goes in. A Seavic pair will outrun you at a walk; ${w($.vehicle?.needs??0)}
    Quarra will not, but they will shift ${w($.capacity??0)} bricks.</p>
    ...
    at a time, up to <b>${f(`well`).well} litres</b>. How fast depends entirely on how well it was built: a poor shaft
    ...
    <p><b>Barrels</b> hold liquid and nothing else, in ${w($r.length)} sizes:
    ${i($r.map((e,t)=>`<b>${e.name.toLowerCase()}</b> (${e.liquid}${t===0?` litres`:``})`))}. One barrel holds one liquid &mdash; water or lye, not
    ...
    <p>Sleeping in a bed banks <b>rest</b> &mdash; ${k(Kt)} the night at a perfect bed and less in a poorer one,
    up to ${j(ke)} of it held at a time. Rest burns only while you are actually working, and everything you
    ...
    for a while &mdash; ${E(1+le)} as much, for anything from ${j(fi)} to ${j(pi)} by how
    ...
    an afternoon of carpentry that teaches you ${E(1+le)} as much.</p>
    <h3>What is actually in a meal</h3>
    <p>Filling the food bar takes a raw potato. Eating <i>well</i> is a different question. ${b(Rn.length)} things
    a body wants are kept separately under the food bar, each fed by different food and each falling
    away on its own over <b>${j(mt)}</b>:</p>
    <table>
      <tr><td><b>Starch</b></td><td>bread, porridge, roots and grain</td></tr>
      <tr><td><b>Flesh</b></td><td>meat and fish &mdash; cooked, meat is worth ${E(H(`cooked_meat`,`flesh`)/H(`meat`,`flesh`))} raw</td></tr>
    ...
    <p>Raw food feeds one of them a little: a potato is ${N(H(`potato`,`starch`))} starch, a piece of
    meat ${N(H(`meat`,`flesh`))} flesh. A <b>cooked dish feeds several, and feeds them properly</b> &mdash; bread is
    ${N(H(`bread`,`starch`))} starch, cooked fish ${N(H(`cooked_fish`,`flesh`))} flesh and ${N(H(`cooked_fish`,`fat`))} fat, and a <b>stew</b> is the only thing on
    the island that feeds all ${w(Rn.length)} at once. Better cooking fills them fuller, so quality tells here as
    everywhere.</p>
    <p>What comes of it: anything in you at all <b>holds hunger and thirst off</b>. On a full board
    they fall at <b>${k(1-Tn)}</b> of their ordinary pace, which is ${j(.5/(Ne*(1-Tn)))} rather
    than ${j(.5/Ne)} before the hunger bar is down to half. And a board with <b>all of them</b> full makes
    everything you do teach you <b>${k(ae)} more</b> &mdash; but that one reads off whichever of them
    ...
    now they have bought nothing at all. One number settles it: <b>a gold coin is worth ${w(ii)}
    silver</b>, every price is named in silver, and change comes back in silver. Paying takes your
    largest coins first, so ${w(ii+1)} silver out of a gold and ${w(ai)} leaves you the ${w(4)} rather than
    ...
    still holds comes back to your purse; one left open for <b>${j(Me)}</b> lapses and gives
    ...
    up the gardens, empty the stores and pull the walls down. There are ${w(ht.length)} standings now.</p>
    <table>
    ...
    <p>The <b>Field guide</b> window (<kbd>F4</kbd>) has a page for each of the ${w(tt().length)} kinds of
    ...
    all ${w(tt().length)}, tamed and bred out of the ${w(tt().filter($e).length)} that
    ...
    takes a kind you have seen only while one is standing within ${w(40)} tiles of you.</p>
    ...
    worth. The copper chisel you washed ashore with is quality ${w(c(`chisel`))}, so it reaches your ceiling
    about one go in ${w(Math.round(100/c(`chisel`)))}. That is the whole reason to better a tool, and every recipe row in
    ...
    <p><b>Everything rots where it lies.</b> A deed slows that to ${k(we)}, a roof over a closed
    room slows it to ${k(Ue)} again, and a crate or a sack slows it further still. A pile of planks
    ...
    <p>Night takes <b>${k(a)}</b> of your sight, which is enough to be a reason to stop walking.
    ${b(tn.length)} things go in your hand against it.</p>
    <p>A <b>torch</b> is ${x(`cloth`,L(`make_torch`,`cloth`))} wound round the head of ${x(`shaft`,L(`make_torch`,`shaft`))} (${Xn(`make_torch`)}) and is
    the poor relation in every way that matters: <b>${w(zn(W))} to ${w(zn(U))} tiles</b> of light
    for <b>${j(Dn(W))} to ${j(Dn(U))}</b>, by how well it was wound, and when it is
    ...
    <p>A <b>lantern</b> (${I(`make_lantern`)} &mdash; ${Xn(`make_lantern`)}) takes a <b>candle</b>
    drawn from beeswax and yarn. A better one keeps the draught off the flame and throws further:
    <b>${w(st(W))} tiles and ${j(Pt(W))}</b> to a candle at the roughest,
    <b>${w(st(U))} tiles and ${j(Pt(U))}</b> at the best.</p>
    ...
    <p>Carrying one lit gives back <b>${k(Ie)}</b> of what the dark takes from your sight, and its
    ...
    <b>oven, kiln or smelter</b> in blast ${w(4)}, and the ${w(Qn.length)} creatures that carry a
    light of their own as far as it reaches &mdash; ${i(Qn.map(e=>`a ${e.name.toLowerCase()} ${w(e.glow??0)}`))}.
    ...
    <b>${j(Bn)}</b> of real time, ${j(Bn/24)} to the game hour. The sun goes down at
    <b>${G(21)}</b> and comes up at <b>${G(3)}</b>, so <b>${k(er)} of it is day and
    ${k(1-er)} night</b> &mdash; ${j(Bn*(1-er))} of real time dark in every day
    &mdash; and the light goes over the ${w(2)} game hours around sundown and comes back over the
    ...
    <b>${G(3+We)}</b>, and the world has not waited for you: fires burn down, crops come on,
    ...
    <p><b>Pick up everything here</b> sweeps the tile you are standing on and the ${w((2*In.SWEEP+1)**2-1)} around it in
    ...
    <p>You carry <b>120 kilos</b> plus <b>${w(5)} kilos</b> for every point of <b>body
    strength</b>, so 220 at the start and ${120+q*5} at ${w(q)}. Going over it costs, and
    the cost climbs with how far over you are: everything past the mark slows you and every action
    takes more wind, as armour does &mdash; ${w(tr)} kilos past a starting back drags on you like about
    ${w(Math.round(Gt(tr,220)/nr))} full suits of iron plate. Past <b>${E(On)}</b> your limit you are down to
    <b>${k(Nt)} of your pace</b>. The inventory footer turns red and says by how much, and the bars
    say so too.</p>
    ...
    is for: a copper tool is good for about ${w(rr(W))} jobs at quality ${W} and ${w(rr(U))} at
    ${w(U)}, and what it is made of stretches or shortens that. Damage also makes a tool work as though it
    were poorer than it is. Past <b>75 damage</b> it warns you in red, and again at every
    ${w(5)} points after; at 100 it breaks and is gone.</p>
    ...
    Taking ${w(ir)} damage out of a tool is ${w(or(K(`repair`)))} goes at repair ${K(`repair`)} and costs it
    ${sr(K(`repair`))} of quality; at repair ${ar} it is ${w(or(ar))} goes for ${sr(ar)}.
    ...
    <p>${b(cr.length)} kinds, and each has a herb that suits it:
    ${i(cr.map((e,t)=>`a <b>${e.name}</b> ${t===0?`wants `:``}<b>${e.herb}</b>`))}. What leaves which
    is what hit you &mdash; a hoof bruises, a claw opens, a sting goes deep and narrow, and
    ${i($n.map(e=>`the ${e.name.toLowerCase()}`))} burn.</p>
    <p>A wound is dressed with a bandage or a cover. A <b>bandage</b> is cloth cut ${w(R(`make_bandage`))} to a length with a knife; a
    <b>healing cover</b> is the herb itself, bruised into cotton, ${w(R(`make_cover_${cr[0].herb}`))} to a batch on the
    <b>first aid</b> skill. Put on cleanly, either stops the bleeding and puts back between ${N(cn(K(`first_aid`),W))} and
    ${N(cn(q,U))} of your health at once, by your first aid and its quality. Right-click either
    ...
    ${E(dt/Vt)} as fast as with nothing on it. The <b>right herb</b> puts back
    ${E(ut)} as much at once, closes it ${E(vt/dt)} as fast as cloth, and it <b>never
    turns</b>; the wrong herb puts back ${E(Bt)} as much and closes it ${N(Zt/dt-1)} faster than cloth.
    Cloth leaves ${N(Sn)} of an open wound's chance of going bad, the wrong herb ${N(Ct)}.
    And every ${w(Math.round(ln/mn))} points of <b>chirurgy</b> closes every wound on you as fast again as
    it closed at none.</p>
    ...
    <p>For <b>${j(Xt)}</b> of real time, whether or not you are playing, it is yours and
    ...
    trade. Every other go comes out at what the tool itself is worth &mdash; ${lr} its quality,
    and never above your skill. The hatchet you washed ashore with is quality ${c(`hatchet`)}, so
    ${k(c(`hatchet`)/U)} of your logs come out at your skill. Nothing you make is ever finer than
    ...
    roll against and comes out at ${lr} your skill, plus 1.</p>
    <h3>Rare things</h3>
    <p>Now and again a thing comes off the bench better than the hands that made it had any right to
    produce. About ${i(J.map((e,t)=>`<b>one thing in ${w(Math.round(1/Et(e)))}</b> ${t===0?`is `:``}<b>${l[e].name}</b>`))}.
    Nothing brings it on &mdash; not skill, not tools, not the metal &mdash; and nothing you do can make
    it more likely; you make ${w(Math.round(1/Et(l.length-1)))} ordinary things and find that you have one.</p>
    <p>A rare thing is <b>better at whatever it was for</b> by ${i(J.map(e=>k(l[e].boost-1))).replace(/ and ([^,]*)$/,` or $1`)}
    &mdash; an edge that bites, armour that turns aside more, a tool that works truer &mdash; <b>wears and rots
    at ${i(J.map(e=>N(l[e].keep)))}</b> of an ordinary one's pace, and can be <b>improved past the
    ceiling of your own skill</b> by ${i(J.map(e=>String(l[e].ceiling))).replace(/ and ([^,]*)$/,` or $1`)}.</p>
    ...
    ${N(Ke)} a step &mdash; ${J.map(e=>`<b>${N(Ke*e)}</b>`).join(`, `)}
    for rare, supreme and fantastic, and never less than <b>one more unit a step</b>, so a kiln's
    ${w(16)} becomes ${i(J.map(e=>w(rn(16,{rare:e}))))}. A rare thing keeps
    ...
    ${j(1/Ft)} it burns, and you can <b>Take ashes</b> from it whether it is lit or cold. They pile up
    ...
    it; on dry land it will not fill. ${_(w(L(`make_lye`,`ash`)))} lots of ashes leached into a bucket of water make a
    ...
    reeds; cut it again too soon and there is nothing left to take. ${_(x(`reed`,L(`make_papyrus`,`reed`)))} soaked in a bucket of
    water, split, laid crosswise and pressed give ${w(R(`make_papyrus`))} sheets of <b>papyrus</b>, which is the
    <b>Papyrusmaking</b> skill.</p>
    <p><b>Ink</b> is the alchemist's part: a gland &mdash; the rare thing off a carcass &mdash; ground with
    ${w(L(`make_ink`,`ash`))} lots of ashes into a bucket of water, for ${w(R(`make_ink`))} lots of ink.
    ${_(w(L(`make_book`,`papyrus`)))} sheets, ${w(L(`make_book`,`leather`))} leather boards, a lot of ink and a needle bind into a <b>book</b>.</p>
    <p>Right-click a book and <b>Study</b> it. Each go takes ${j(je(ur))}, raises <b>mind logic</b>, which is
    ...
    <b>title</b> at ${i(oe.map(String))} &mdash; ${s(`carpentry`).map(e=>e.name).join(`, `)}
    ...
    <p>A <b>knack</b> comes of the work itself: <b>one go in ${w(kn)}</b>, at any trade and at any
    level, leaves one behind. It lands on the trade you were working ${k(ie)} of the time, and
    otherwise on one of the trades beside it &mdash; a long day of carpentry may leave you better at
    bowyery, because it is the same hands and the same wood. A knack is worth ${k(Be)} more on
    everything that trade teaches you from then on, it never wears off, and a trade holds
    <b>${w(5)}</b> of them: ${E(1+5*Be)} on every gain, for good. They stack with a night's rest
    ...
    <p>A cooked dish leaves a knack too, and a stronger one &mdash; ${E(1+le)} rather than
    ${k(Be)} more &mdash; but it wears off, in ${j(fi)} to ${j(pi)}. One is earned and kept;
    ...
    hold it &mdash; without the lye it washes straight out. ${b(Ze.length)} of them, easiest first:
    ${i(dr.map(e=>`<b>${e.name.toLowerCase()}</b> (${e.word}, from ${x(e.from,e.count)}, alchemy ${e.difficulty})`))}.
    One boil gives ${w(R(`make_${Ze[0].id}`))} pots and hands the bucket back.</p>
    ...
    ${w(L(`make_rope`,`wemp`))} fibres to a rope, on the <b>ropemaking</b> skill. ${_(x(`rope`,L(`make_thick_rope`,`rope`)))} laid up again make a <b>thick
    rope</b>, which is the hawser everything heavy hangs on.</p>
    <p>Rope is not decoration. A <b>bridle</b> takes ${w(L(`make_bridle`,`rope`))} for the reins, a <b>rowing boat</b>
    ${w(L(`make_rowing_boat`,`rope`))}, a <b>sailing boat</b> ${w(L(`make_sailing_boat`,`rope`))} and ${w(L(`make_sailing_boat`,`thick_rope`))} hawsers for her standing
    rigging, and a <b>well</b> ${x(`thick_rope`,L(`make_well`,`thick_rope`))} to hang the bucket down the shaft. Keep a field of
    ...
    <b>yarn</b>, and at the loom to weave ${w(L(`weave_cloth`,`yarn`))} yarn into a length of <b>cloth</b>. Cloth stuffs a
    mattress, sews into clothing, and twisted into a <b>bowstring</b> it is the start of every bow.</p>
    <h3>Armour</h3>
    <p>Armour is worn a piece at a time in ${w(fr.length)} places &mdash; ${i(fr)} &mdash;
    ...
    <p>There are ${w(pr.length)} kinds anybody can make, each with a skill of its own that rises <b>by being
    hit in it</b>, and each turns aside a share of a blow before its quality, its metal and that skill:
    <b>cloth</b>, sewn by tailoring, ${N(A.cloth.soak)}; <b>leather</b>, cut from tanned hide with a knife
    by leatherworking, ${N(A.leather.soak)}; <b>chain</b>, poured from moulds and riveted up at an anvil by
    chain armoursmithing, ${N(A.chain.soak)}; and <b>plate</b>, beaten out whole by plate armoursmithing,
    ${N(A.plate.soak)}. Quality and the skill behind it raise all of those, and damage lowers them: armour
    wears where it is struck, and a piece beaten to nothing falls off you. Weight is the price &mdash; a full
    suit of iron plate slows you by ${N(nr/(1+nr))} and makes every action cost ${N(nr)} more
    wind, where cloth costs ${N(A.cloth.burden)}.</p>
    ...
    <p>Every weapon belongs to a kind, and each kind is its own subskill: ${i(mr.map(e=>`<b>${e}</b>`))}.
    ...
    ${w(gt.find(e=>e.id===`arrow_head_mould`)?.per??0)} <b>arrow heads</b>. A club is simply carved from a log, which is what most people start with.</p>
    <p>Bows are tillered with <b>bowyery</b> from shafts and a bowstring, in ${w(hr.length)} sizes:
    ${i(hr.map((e,t)=>`a <b>${S(e.id).name.toLowerCase()}</b> ${t===0?`reaches `:``}${w(e.range??0)}${t===0?` tiles`:``}`))},
    each slower to draw and heavier in the hit than the last. Arrows are made with <b>fletching</b> from
    ${I(`make_arrows`)}, ${w(R(`make_arrows`))} to a go &mdash; and feathers come only off a bird:
    ${i(gr.map(e=>`the ${e.name}`))}. With a bow in hand, <b>Shoot</b> appears on any wild creature
    ...
    walls, smelters and kilns are built from &mdash; or, ${w(L(`make_stone_slab`,`rock_shards`))} shards at a time, a <b>slab</b>. Slabs are
    not for building: they are paving. Choose <b>Pave (slabs)</b> on any tile with a trowel in hand and
    the slab goes down as a floor of that stone, and each of the ${w(Kn.length)} looks quite different from the
    ...
    <b>Repair</b> takes out. There are ${w(h.length)} things under the island, from ${it(h[0].name)} <b>${h[0].name}</b> in
    ${w(h[0].parts)} pieces to ${it(Y.name)} <b>${Y.name}</b> in ${w(Y.parts)}, and a relic is only recognised once your archaeology has
    ...
    its mining reaches ${_r(`iron`)}. It works <b>${w(10)} units of water</b> deep, the same as you do, and a face it
    ...
    <b>shear</b> it with a knife for <b>wool</b>, which grows back in ${j(1/(D.woola.fleece??1))}. The
    <b>Ulva</b> is the first thing on this island that will come at you unprovoked: it hunts by scent
    from ${w(D.ulva.notice??7)} tiles off and does not stop until you are well away or it is badly hurt. It takes taming
    ...
    <b>mount</b> it. A green one carries you at ${E(vr/Rt)} your own pace and over the same ground;
    one whose climbing is worked right up is ${E(yr/vr)} as fast as a green one and goes up slopes you would
    ...
    both its bite and its beat: it hits ${E(zt(q))} as hard at mastery, and its circuit grows from
    ${w(D.rowl.workRange)} tiles to ${w(D.rowl.workRange+nn(q)*(D.rowl.rangePerStep??10))}. The <b>Noot</b> is a plump upright waddler that
    ...
    readier for the next offering: <b>${N(fn)} on the chance</b> for every attempt in a row, with no
    ...
    ${j(xe)}, and <b>raising a hand to it ends the run outright</b> &mdash; nothing that has been hit
    ...
    ${N(v.young.scale)} the size, moves at ${N(v.young.speed)} of the pace, grows no fleece and gives no milk, and is no use in the
    traces or under a saddle &mdash; but it has not learned to mistrust you, so it is ${E(v.young.tame)} as easy
    to tame. It is <b>grown</b> after ${j(yt)}, and everything in the book describes it then. After
    ${j(an)} it is <b>old</b>: ${N(v.old.speed)} of the pace, ${N(v.old.pull)} of the pull in the traces and
    ${N(v.old.growth)} of the pace growing a fleece back, but heavier, and an old carcass is worth ${N(v.old.yield-1)} more
    ...
    than all of them: each adds ${N(D.shaggan.pull??.25)} to a team's pull where most beasts add ${N(pn)}.</p>
    ...
    empty bucket, and a bucket of milk presses into ${w(R(`make_cheese`))} <b>cheeses</b>. The <b>Vesp</b> is a swarm
    ...
    <b>beeswax</b> &mdash; and ${I(`make_candle`)} draw ${w(R(`make_candle`))} <b>candles</b>. The
    <b>Lume</b> is only ever out after dark and carries its own light about with it: keep one and it
    lights ${w(D.lume.glow??0)} tiles round itself however dark it is.</p>
    ...
    border out ${w(2)} tiles and lets ${ue(2)-ue(1)===1?`one`:w(ue(2)-ue(1))} more wildermon work the deed, and each is earned by building the
    settlement out: ${br}. Upgrades are
    ...
    <p>A deed worker feeds itself: once its belly falls below ${k(nt)} it goes to whichever crate on
    ...
    <p>A deed worker starts within ${w(xr[0])} to ${w(xr[xr.length-1])} tiles of the token, by its sort, and earns another
    10 tiles of range for every 10 levels of its task skill &mdash; ${Sr} &mdash; so a
    ...
    with a mallet from ${r}; it weighs <b>${S(gn).weight} kg</b> and does not rot.
    ...
    <b>one more pass over the tile for every ${w(20)} points</b> of the skill: ${Cr}.
    ...
    sit, and choose <b>Sit and think about nothing</b>. You may sit once every ${j(Fn)}, and
    <b>where</b> you sit decides what it is worth: your own yard ${N(Ve.yard)} of a sitting anywhere else,
    high ground off your settlement ${E(Ve.high)} as much, and where the ground runs out and the air is thin
    <b>${E(Ve.thin)}</b> as much; and your feet in the water add ${N(Ve.water-1)} to wherever that is.</p>
    <p>At ${w(5)} meditation ${w(C.length)} ways of looking at the island become clear and you may walk exactly
    <b>one</b>, chosen at the rug and never changed. Each opens ${w(C[0].steps.length)} things as the sitting goes on:
    ${w(C[0].steps.filter(e=>e.ability).length)} of them are abilities you call on with a rest between, and ${w(C[0].steps.filter(e=>!e.ability).length)} are simply true from then on.</p>
    <table>
      ${C.map(e=>`<tr><td><b>${e.name}</b></td><td>${e.note} ${e.steps.map(e=>e.ability?`<b>${e.name}</b> (${e.at}, then ${j(e.ability.rest)} before it again): ${wr(e.note)}`:`<i>${e.name}</i> (${e.at}): ${wr(e.note)}`).join(` `)}</td></tr>`).join(`
      `)}
    </table>
    ...
    <b>favour</b>, on the <b>prayer</b> skill. You may say what you have to say once every ${j(Ln)}, and it is
    worth most at <b>${i(wn.map(G))}</b> &mdash; ${E((Mt+at)/Mt)} what it is worth
    ${w(3)} hours or more from either &mdash; and less the further off you are. A good altar banks more
    than a rough one. Favour also trickles back on its own, slowly, up to whatever your faith carries
    &mdash; ${Math.round(pt(K(Lt)))} at the start and 120 at the very top.</p>
    <p>${b($t.length)} things it buys, and none of them can be had any other way:</p>
    <table>
      ${$t.map(e=>`<tr><td><b>${e.name}</b></td><td>${e.cost} favour, prayer ${e.level}. ${e.note}</td></tr>`).join(`
      `)}
    </table>
    <h3>The things that are not wildermon</h3>
    <p>Most of what walks this island can be tamed. ${b(Tr.length)} things cannot. A <b>goblin</b> is knee-high and
    ...
    They notice you from ${w(Er[0])} to ${w(Er[Er.length-1])} tiles off, where anything else that hunts has your scent at
    ${w(7)}; they come straight at you, and they do not give up easily.</p>
    <p>They are <b>rare</b>: ${N(vn)} of what stands up out in the country, and of those
    ${i(g.map(([e,t])=>`${N(t/Dr)} ${D[e].name.toLowerCase()}s`))}. Only so many of each are alive at once
    (${i(g.map(([t])=>`${w(e[t]??1)} ${D[t].name.toLowerCase()}${(e[t]??1)===1?``:`s`}`))}), and the bigger the thing the further it keeps
    from your token &mdash; ${i(g.map(([e])=>`${it(D[e].name)} ${D[e].name.toLowerCase()} ${w(Dt[e]??0)} tiles`))}. None of them can be tamed,
    trapped, bred or brushed. There is nothing to be done with one but kill it, and nothing to be gained
    by meeting one in your shirt: ${i(g.map(([e],t)=>`${it(D[e].name)} ${D[e].name.toLowerCase()}'s ${t===0?`blow takes `:``}${N(Math.round(D[e].attack*Ye*100)/100)}`))} of an unarmoured life.</p>
    <p>What they are worth is on the other side of that. Butchering one gives what a wildermon gives and
    then some: <b>tusk</b> and <b>sinew</b> off an orc or an ogre, and off a dragon ${w(D.dragon.butcher.scale??0)}
    <b>dragon scales</b> and a <b>hoard</b> &mdash; ${w(4)} to ${w(10)} lumps, by how much of the carcass you keep,
    of the ${w(Or.length)} deep metals and the ${w(ze.length-Or.length)} precious ones, which is the only place on the
    island they turn up together.</p>
    <p>Tusk and sinew make a <b>composite bow</b>: ${I(`make_composite_bow`)}, at ${Xn(`make_composite_bow`)}. It
    throws an arrow <b>${w(kr.range??0)}</b> tiles for ${kr.damage} damage, where the best wooden bow throws
    ${w(Ar.range??0)} for ${Ar.damage}. Dragon scale riveted to leather makes <b>scale armour</b>, a class above
    plate: it turns <b>${N(A.scale.soak)}</b> of a blow where plate turns ${N(A.plate.soak)}, and it burdens you
    less than chain. A full suit takes ${w(jr)} scales, where one dragon carries ${w(D.dragon.butcher.scale??0)}.</p>
    <h3>Traps</h3>
    ...
    <p>A <b>snare</b> is a noose of rope on a bent shaft: ${p(d.snare.bill)}, and it holds anything
    up to about <b>taming ${d.snare.holds}</b>. A <b>deadfall</b> is a weighted board on a trigger &mdash;
    ${p(d.deadfall.bill)}, with a mallet &mdash; and it holds to about <b>taming ${d.deadfall.holds}</b>.
    ...
    <p>A trap rots where it stands, ${j(d.snare.lifeMin)} to ${j(d.snare.lifeMax)} for a snare and up to
    ${j(d.deadfall.lifeMax)} for a good deadfall, and whatever is in it walks away when it goes over. A <b>timid</b>
    creature &mdash; the very thing you cannot walk up to &mdash; is ${E(qe)} as likely to walk into
    one, and a hunter ${N(te)} as likely. Getting the catch out is still <b>taming</b>: the skill wall stands
    ...
    traits are the whole difference between one Roxxen and the next. A trait sits in one of ${w(m.length)} tiers
    &mdash; ${i(m.map(e=>`<b>${e}</b>`))} &mdash; and what it is worth
    climbs steeply with the tier: a common trait moves its number by ${N(Mr.common[0])} to ${N(Mr.common[1])},
    a fantastic one by ${N(Mr.fantastic[0])} to ${N(Mr.fantastic[1])}. Traits lift how fast it <b>moves</b>, how quickly it <b>works</b>, how fast what it does
    ...
    <p>${b(Ht.length)} of the traits are <b>fighting blood</b> &mdash; fanged, plated, slippery, quick-jawed,
    ...
    (rare), fanged (supreme) or fanged (fantastic), each worth steeply more than the last &mdash; ${i(m.map(e=>E(Ee[e])).slice(1))} what
    the common grade is &mdash; and a line is bred up a grade at a time under the same husbandry.
    ${_(N(Te))} of what the wild throws up is fighting blood, and no animal carries more than one grade of a name.</p>
    ...
    works quicker, learns faster and heals as you go over it. Care runs out again over ${j(3*Zn)}
    ...
    both <b>grown</b>, both <b>fed</b>, and neither put to a mate in the last ${j(pe)}, then
    ...
    the female carries for ${j(Qe)} and then drops a young one, and what it is born with was
    settled at that moment &mdash; a sire sold, released or eaten in between has already had his say.</p>
    <p>${_(w(3))} slots are filled one at a time. Each is drawn from what the pair carry between them, and
    husbandry decides <b>how often the parents' blood comes through</b> rather than whatever the wild throws up
    (${k(Se(0,0))} at no skill, ${N(Se(q,1))} at ${w(q)} with a well-brushed pair), and <b>how often a
    trait comes through one tier better than either parent had it</b> (never at no skill and with no care,
    ${N(Le(q,1))} at ${w(q)} with a well-brushed pair) &mdash; commons becoming rares, rares becoming supremes.
    ...
    enough to recognise: common blood is plain to anybody, ${i(m.slice(1).map((e,t)=>`${e} ${t===0?`takes `:``}${rt[e]+1}`))}
    ...
    <p><b>A spadeful out of the cart.</b> Dirt, clay and sand weigh <b>${S(`dirt`).weight}kg</b> apiece, so a starting
    back carries ${w(Math.floor(220/S(`dirt`).weight))} before it starts to drag, and moving a bank is a great many walks. Dropping dirt and the packing-in half
    ...
    worked at all: ${i(X.filter(e=>e.level===X[0].level).map(e=>Nr(e.name)))} from the very start, then
    ${i(X.filter(e=>e.level!==X[0].level).map(e=>`${Nr(e.name)} at ${e.level}`))}. <b>Iron is
    ${N(Nn(`iron_ore`))} of every seam on the island</b> and everything else shares what
    ...
    ${Math.round(1/Re)}</b> a slab comes away of its own accord and the corner drops a
    ...
    than working it, and it gives way about one attempt in ${w(Math.round(1/Ce))}. What breaks away is yours either
    way.</p>
    ...
    ${w(Math.round(1/Cn.land))} carries some seam, and one sea tile in ${w(Math.round(1/Cn.water))}. <b>Prospecting</b> is how you read it. It marks every
    ...
    ${i(Pr.map((e,t)=>`<b>${Ir(e)}</b> at ${t===0?`level `:``}${e}`))} &mdash;
    ${w(Math.round(1/Fr(Pr[0])))} goes for the first point of a skill, ${w(Math.round(1/Fr(Pr[2])))} for the point after
    ${Pr[2]}, and something like <b>${w(Math.round(1/ft))}</b> for the last. Nobody finishes a skill in passing; the last
